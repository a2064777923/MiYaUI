...
class Sliders{
    ...
    public static function slider_1($data,$i){
        ...
        $slider_html ='<div class="slider-1 carousel slider-height b2-radius '.($data['slider_width'] === '0' ? 'slider-type-width' : '').' '.($data['slider_show_title'] ? 'slider-show-title' : 'show-title-none').'" '.$slider_setting.' style="'.((int)$data['slider_width'] == 1 || $data['show_widget'] ? 'width:'.$data['width'].'px' : 'width:100%').';max-width:100%">
        '.$html.'
    </div>';
    return apply_filters('b2_slider_1_html' , $slider_html);
    }
     ...
     * @since 2024
     */
    public static function slider_7($data,$i){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $html .= '<div class="slider-1-carousel slider-height wrapper b2-radius" style="max-width:100%;">
                    <div class="slider-in slider-info b2-radius" style="height:0;padding-top:'.round($data['slider_height']/$data['width']*100,6).'%;max-width:100%">
                        '.b2_get_img(array('src'=>$thumb,'class'=>array('slider-img','b2-radius'))).'
                        <a class="link-block" '.(!$data['slider_new_window'] ? 'target="_blank"' : '').'  href="'.($v['link'] ? $v['link'] : 'javascript:void(0)').'"></a>
                        '.($data['slider_show_title'] ? $cat : '').'
        ...
        $slider_html ='<div class="jitheme-slider slider-1 carousel slider-height b2-radius '.($data['slider_width'] === '0' ? 'slider-type-width' : '').' '.($data['slider_show_title'] ? 'slider-show-title' : 'show-title-none').'" '.$slider_setting.' style="'.((int)$data['slider_width'] == 1 || $data['show_widget'] ? 'width:'.$data['width'].'px' : 'width:100%').';max-width:100%">
        '.$html.'
    </div>';
    return apply_filters('b2_slider_1_html' , $slider_html);
    }
     ...
     * @param array $data 设置项参数
     *
     * @return string 幻灯的htmlflickity-button
     * @author Li Ruchun <lemolee@163.com>
     * @version 1.0.0
     * @since 2018
     */
public static function slider_8($data,$i){
        ...
        $css = '
        <style>
        .jitheme-slider-8-'.$i.' .jitheme-slider-8-bottom .slider-in:nth-child(1),.jitheme-slider-8-'.$i.' .jitheme-slider-8-bottom .slider-in:nth-child(2){
            margin-bottom:'.$data['slider_gap'].'px;
        }
        .jitheme-slider-8-'.$i.' .jitheme-slider-8-bottom .slider-height{
            margin-right:'.$data['slider_gap'].'px;
        }
        .jitheme-slider-8-'.$i.' .jitheme-slider-8-bottom{
            margin-right:-'.$data['slider_gap'].'px;
        }
        </style>
        ';
        ...
        foreach($post_data as $k => $v){
            ...
            $cat = self::get_cat_info($v);
            $html_slider .= '
            <div class="jitheme-slider-8-carousel slider-height" style="max-width:100%;height:100%">
                <div class="slider-info b2-radius">
                    <a class="link-block" '.(!$data['slider_new_window'] ? 'target="_blank"' : '').'  href="'.($v['link'] ? $v['link'] : 'javascript:void(0)').'"></a>
                    '.b2_get_img(array('src'=>$thumb,'class'=>array('slider-img','b2-radius'))).($data['slider_show_title'] ? 
                    $cat
        ...
        //获取后4篇文章
        $html_last = '<div class="jitheme-slider-8-bottom" style="width:'.$c_right.'%">';
        $h = round(($data['slider_height']-$data['slider_gap'])/2,0);
        foreach($last_arr as $k => $v){
            $thumb = b2_get_thumb(array(
                'thumb'=>$v['thumb'],
            ...
            $html_last .= '
            <div class="slider-in b2-radius">
                <div class="jitheme-slider-8-right-item slider-height" style="height:0;padding-top:'.round($h/$widget_width*100,6).'%">
                    <div class="slider-info b2-radius box">
                        <a class="link-block" '.(!$data['slider_new_window'] ? 'target="_blank"' : '').'  href="'.($v['link'] ? $v['link'] : 'javascript:void(0)').'"></a>
                        '.b2_get_img(array('src'=>$thumb,'class'=>array('slider-img','b2-radius'))).($data['slider_show_title'] ? $cat : '').'
                    </div>
        ...
        return '<div class="jitheme-slider-8 jitheme-slider-8-'.$i.'  '.($data['slider_show_title'] ? 'slider-show-title' : 'show-title-none').'" style="width:var(--ji-wrapper-width);max-width:100%">'.$html_slider.$html_last.'</div>';
    }
