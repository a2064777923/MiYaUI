...
use B2\Modules\Common\StableSort;
...
class Circle{
    ...
    public static function insert_topic_card($id){
        ...
        if($post_type && ($post_type == 'post' || $post_type == 'shop' || $post_type == 'document' || $post_type == 'newsflashes' || $post_type == 'announcement' || $post_type == 'page')){
            $thumb_url = \B2\Modules\Common\Post::get_post_thumb($id);
    ...
    public static function get_circle_data($circle_id,$current_circle_id = 0){
        ...
        if(isset($GLOBALS['b2_circle_data_'.$circle_id.'_'.($current_circle_id ? 'show' : 'hidden')])) return $GLOBALS['b2_circle_data_'.$circle_id.'_'.($current_circle_id ? 'show' : 'hidden')];
        ...
        $circle_data = array(
            ...
            'user_count'=>$default == $c->term_id ? User::user_count() : self::user_count_in_circle($c->term_id),
            'topic_count'=>$default == $c->term_id ? wp_count_posts('circle')->publish : $c->count,
        ...
        $GLOBALS['b2_circle_data_'.$circle_id.'_'.($current_circle_id ? 'show' : 'hidden')] = $circle_data;
    ...
    public static function is_circle_admin($user_id,$circle_id){
        ...
        if(isset($GLOBALS['b2_is_circle_admin_'.$user_id.'_'.$circle_id])) return $GLOBALS['b2_is_circle_admin_'.$user_id.'_'.$circle_id];
        ...
        $GLOBALS['b2_is_circle_admin_'.$user_id.'_'.$circle_id] = $isset;
    ...
    public static function is_circle_member($user_id,$circle_id){
        ...
        if(isset($GLOBALS['b2_is_circle_member_'.$user_id.'_'.$circle_id])) return $GLOBALS['b2_is_circle_member_'.$user_id.'_'.$circle_id];
        ...
        $GLOBALS['b2_is_circle_member_'.$user_id.'_'.$circle_id] = $isset;
    ...
    public static function is_circle_pending($user_id,$circle_id){
        ...
        if(isset($GLOBALS['b2_is_circle_pending_'.$user_id.'_'.$circle_id])) return $GLOBALS['b2_is_circle_pending_'.$user_id.'_'.$circle_id];
        ...
        $GLOBALS['b2_is_circle_pending_'.$user_id.'_'.$circle_id] = $isset;
    ...
    public static function get_topic_list($data){
        ...
        if(isset($data['count']) && (int)$count <= 50){
    ...
    public static function get_data_by_topic_id($topic_id,$user_id = 0,$topic_query = null){
        ...
        if($status == 'pending'){
            //新增钩子 帖子待审
            do_action('new_circle_pending', get_post($topic_id));
        ...
        if($data['type'] === 'hot'){
            ...
            if($role){
                ...
                $res = $wpdb->get_results(
                    $wpdb->prepare("SELECT * FROM $table_name WHERE `user_id`=%d AND `circle_role`!=%s",$user_id,'pending')
                ,ARRAY_A);
    ...
    public static function answer_count($topic_id){
        ...
        $count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table_name WHERE post_parent=%d AND post_type=%s",$topic_id,'circle_answer'));
    ...
    public static function answer_right($answer_id){
        ...
        Message::update_data([
            ...
            'to'=>$answer,
            'post_id'=>$parent,
            'msg'=>sprintf(__('您的回答被${from}采纳了：${post_id}','b2'),$pay),
            'type'=>'best_answer',
