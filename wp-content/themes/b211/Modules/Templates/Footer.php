    ...
    public static function footer_menu_str_to_html($str){
        ...
        foreach ($str as $k => $v) {
            ...
            if($v === 'login'){
                ...
                if(strpos($link,'${avatar}') !== false){
                    $link = str_replace('${avatar}', '\'+userData.avatar+\'', $link);
                    $must_login = true;
                }

                if(strpos($link,'${name}') !== false){
                    $link = str_replace('${name}', '\'+userData.name+\'', $link);
                    $must_login = true;
                }

                if(strpos($link,'${id}') !== false){
                    $link = str_replace('${id}', '\'+userData.id+\'', $link);
                    $must_login = true;
                }

                if(strpos($link,'${link}') !== false){
                    $link = str_replace('${link}', '\'+userData.link+\'', $link);
                    $must_login = true;
                }

                if(strpos($link,'${code}') !== false){
                    $link = str_replace('${code}', '\'+userData.user_code+\'', $link);
                    $must_login = true;
                }

                if($must_login){
                    $arg[] = '<a :href="userData.id ? \''.$link.'\' : \'javascript:void(0)\'" onclick="b2mobileFooterMenu.userData.id ? \'\' : (login.show = true)"><span>'.$icon.'</span><b>'.$text.'</b></a>';
                }else{
