 ...
 */
    ...
    if(isset($_GET['collection']) && !empty($_GET['collection'])){
        $settings['collection_slug'] = array(sanitize_text_field($_GET['collection']));
    }

    //标签筛选
    if(isset($_GET['post_tag']) && !empty($_GET['post_tag'])){
        $settings['post_tag'] = array(sanitize_text_field($_GET['post_tag']));
    }
    ...
    if(isset($_GET['post_order']) && !empty($_GET['post_order'])){
        $settings['post_order'] = sanitize_text_field($_GET['post_order']);
    }

    //搜索
    if(isset($_GET['archiveSearch']) && !empty($_GET['archiveSearch']) ){

        $settings['search'] = sanitize_text_field($_GET['archiveSearch']);
    }
