...
class Newsflashes
{
    ...
    public static function submit_newsflashes($data)
    {
        ...
        $open = b2_get_option('newsflashes_main', 'newsflashes_can_post_open');
        if ($open == 0 && !user_can($user_id, 'manage_options')) {
            return array('error' => __('不允许发布', 'b2'));
        }
        ...
        if (!$role && !user_can($user_id, 'manage_options') && !user_can($user_id, 'editor'))
            return array('error' => sprintf(__('您没有权限发布%s', 'b2'), $newsflashes_name));
