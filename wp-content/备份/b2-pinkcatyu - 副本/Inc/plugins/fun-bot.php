...
function request_by_curl($remote_server, $post_string) {  
	...
	if(lmy_get_option('bot_api_comment')){
...
function boxmoe_msg_comment($comment_id){
    ...
    $message = $text . "\n" . "作者: $comment->comment_author \n邮箱: $comment->comment_author_email \n评论: $comment->comment_content \n 点击查看：$siteurl/?p=$comment->comment_post_ID#comments";
		if(lmy_get_option('bot_api_server') == 'boe_api_qq' ){
			$apiurl	=	lmy_get_option('bot_api_url');
			$msgqq	=	lmy_get_option('bot_api_qqnum');
		...
		if(lmy_get_option('bot_api_server') == 'boe_api_dd' ){
			$time    = intval(microtime(true) * 1000);
			$secret  = lmy_get_option('bot_api_ddkey');
			$sign    = urlencode(base64_encode(hash_hmac('sha256', "{$time}\n{$secret}", $secret, true)));
			$apiurl	=	lmy_get_option('bot_api_url').'&timestamp='.$time.'&sign='.$sign;
