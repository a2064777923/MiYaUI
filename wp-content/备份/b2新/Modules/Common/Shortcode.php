...
class Shortcode
{
    ...
    public function init()
    {
        if (!is_admin()) {
            ...
            add_filter('the_content', [__CLASS__, 'remove_hidden_content_for_feed'], 20);
            add_filter('the_excerpt_rss', [__CLASS__, 'remove_hidden_content_for_feed'], 20);
            add_filter('the_content_feed', [__CLASS__, 'remove_hidden_content_for_feed'], 20);
        }
    ...
    public static function remove_hidden_content_for_feed($content)
    {
        if (is_feed()) {
            $patterns = [
                "/\[content_hide\](.*?)\[\/content_hide\]/is",
                "/\[b2_content_hide\](.*?)\[\/b2_content_hide\]/is"
            ];
    ...
    public static function custom_pay($atts)
    {
        ...
        $show_res_list = apply_filters(
            'b2_cpay_tabs_' . $atts['id'],
    ...
    public static function get_content_hide_arg($post_id, $order_id = '', $json = false, $user_id = 0)
    {
        ...
        if (
            preg_match_all('/' . $pattern . '/s', get_post_field('post_content', $post_id), $matches)
    ...
    public static function check_reading_cap($post_id, $user_id, $order_id = '')
    {
        ...
        if (isset($cap['user_id']))
            return array('error' => __('没有数据', 'b2'));
        $allow = false;
