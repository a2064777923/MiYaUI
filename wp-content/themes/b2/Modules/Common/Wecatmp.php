...
class Wecatmp
{
    ...
    public static function get_unionid_by_openid($openid)
    {
        $token = self::get_token(); //获取access_token
        if (isset($token['error']))
            return $token;

        return self::remote_get('https://api.weixin.qq.com/cgi-bin/user/info?access_token=' . $token . '&openid=' . $openid . '&lang=zh_CN');
    }
    ...
    public static function mp_login($code)
    {
        ...
        if ($save_sence_id === false)
            return array('error' => __('参数错误', 'b2'));

        if ((int) $save_sence_id === 1)
            return 'waiting';

        $unionid = self::get_unionid_by_openid($save_sence_id);

        $check = OAuth::check_binding(array('uid' => $save_sence_id,'unionid' => $unionid['unionid'] ?? '', 'type' => 'weixin'));
    ...
    public static function message_success($data)
    {
        ...
        if (!b2_get_option('normal_weixin_message', 'weixin_message_open'))
            return 'close';
