...
class Jitheme_widget_hotask extends WP_Widget{

    //小工具slug
	protected $widget_slug = 'ji-widget-ask';

    //短代码名
	protected static $shortcode = 'ji_widget_ask_post';
	...
	public function __construct() {
		...
		add_shortcode( self::$shortcode, array( __CLASS__, 'ji_widget_ask_post' ) );
	}
	...
	public static function get_widget( $atts ) {
				...
				<div v-if="data !== \'\'" v-cloak>
					...
					<div class="widget-content"  v-else>
                        <div class="t-item" v-for="(item,i) in data.data">
                            <a :href="item.link" target="_blank" class="item-widget"><i class="item-index"><i :class="`icon-fire-${i + 1}`" class="ico"></i></i>
                                ...
                                <div class="item-meta"><span v-text="item.metas.answer_count"></span>'.__('位网友参与回答','b2').'</div>
                            </a>
                        </div>
					</div>
				</div>
		...
		if ( empty( $widget )) {
			...
			$widget .= '</div>';
			$widget .= '<div  id="jitheme-widget-ask" class="b2-widget-box">'.$html.'</div>';
			$widget .='<div class="jitheme-widget-btn "><a class="jitheme-jb-btn" href="/ask" target="_blank">全部问答</a></div></div>';
...
function Jitheme_widget_hotask() {
	register_widget( 'Jitheme_widget_hotask' );
}
add_action( 'widgets_init', 'Jitheme_widget_hotask' );