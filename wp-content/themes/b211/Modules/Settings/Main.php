...
class Main
{
    public function init()
    {
        ...
        if (!apply_filters('b2_check_role', 0))
            return;

        $data = apply_filters('b2_user_info', []);

        if (isset($data['can']) && !$data['can']) {
            return;
        }
    ...
    public function sample_admin_notice__success()
    {
        ...
        if (!$status) {
            ...
            <div class="notice notice-success is-dismissible">
                <p>
                    <?php echo sprintf(__('B2Pro 主题未激活，请先正确安装并%s激活主题%s后再使用', 'b2'), '<a href="' . admin_url('/wp-admin/admin.php?page=b2_main_options') . '">', '</a>'); ?>
                </p>
            </div>
            <?php
        }
    ...
    public function cmb2_render_callback_for_text_two($field, $value, $object_id, $object_type, $field_type)
    {
        ...
        ?>
        <div>
            <p><label for="<?php echo $field_type->_id('_address_1'); ?>">
                    <?php echo __('距离上一个模块高度', 'b2'); ?>
                </label></p>
        ...
        <div>
            <p><label for="<?php echo $field_type->_id('_address_2'); ?>'">
                    <?php echo __('距离下一个模块高度', 'b2'); ?>
                </label></p>
            <?php echo $field_type->input(
                array(
    ...
    public function check_cg()
    {
        ...
        if (defined('B2_THEME_DEBUG') && B2_THEME_DEBUG)
            return '';
        ...
        if ($p_v[0] < '7.3' || $p_v[0] > '8.4') {
            $text = '<h2 class="red">请更换PHP版本</h2><p>请使用  PHP7.3 - PHP8.4 版本，当前版本为 ' . PHP_VERSION . '</p>';
        } else {
            ...
            $directory = B2_THEME_DIR . B2_DS . 'Modules' . B2_DS . 'Common' . B2_DS . 'Private' . B2_DS . $php_v;
            $directory = PATH_SEPARATOR != ':' ? str_replace('/', B2_DS, $directory) : $directory;

            $_text = '<h2 class="red">请安装扩展</h2>
                        <p>' . __('未安装扩展，请按照下面的方法进行安装', 'b2') . '</p>
                        <p>' . sprintf(__('1、打开您的php.ini文件（%s）', 'b2'), '<code>' . php_ini_loaded_file() . '</code>') . '</p>
                        <p>' . sprintf(__('2、将下面的代码复制到php.ini文件的最后一行保存：%s', 'b2'), '<br /><br /><code>[swoole_loader]</code><br /><code>extension=' . $path . B2_DS . 'Assets' . B2_DS . 'admin' . B2_DS . 'loader' . B2_DS . $loader_name . '</code>') . '</p>
                        <p>' . __('3、重启php', 'b2') . '</p>
                        <p>' . __('4、刷新本页', 'b2') . '</p>';
            ...
            if (!extension_loaded('swoole_loader')) {
                ...
                if (!($data['can'] ?? false)) {
                    ...
                    if ($this->is_ip($domain)) {
                        $text = '<h2 class="red">不支持IP</h2><p>不支持IP，如果您WP后台设置的是IP，请将wp设置中的IP改成已经在7b2官网登记的域名</p>';
                    } else {
                        $text = '<h2 class="red">未授权</h2><p>您当前使用的域名 <b class="red">' . $domain . '</b> 未授权，主题无法正常使用，请先在<a href="https://7b2.com/check/register" target="_blank">7b2官网</a>登记授权后再使用。</p>
                        <p>登记完毕后，需要重新下载主题，并安装到您的wp站点中。</p>';
                    }
                }
                ...
                if (extension_loaded('swoole_loader')) {
                    ...
                    if ($ver != '3.2.1') {
                        $text = '<h2 class="red">请升级扩展</h2>
                        <p>' . sprintf(__('1、打开您的php.ini文件（%s）', 'b2'), '<code>' . php_ini_loaded_file() . '</code>') . '</p>
                        <p>' . sprintf(__('2、php.ini底部找一下，如果以前添加过类似%s这样的代码，先将其删掉，如果没有就下一步', 'b2'), '<code>extension=***' . B2_DS . $loader_name . '</code>') . '</p>
                        <p>' . sprintf(__('3、将下面的代码复制到php.ini文件的最后一行保存：%s', 'b2'), '<br /><br /><code>[swoole_loader]</code><br /><code>extension=' . $path . B2_DS . 'Assets' . B2_DS . 'admin' . B2_DS . 'loader' . B2_DS . $loader_name . '</code>') . '</p>
                        <p>' . __('4、重启php', 'b2') . '</p>
                        <p>' . __('5、刷新本页', 'b2') . '</p>';
                    }
                ...
                //     $text = '<h2 class="red">暂时不支持该系统</h2>
                //     <p>因扩展兼容问题，当前版本暂未支持 Windows 系统，预计25年9月份以后支持。请使用Linux系统，并安装PHP8.0以上版本。</p>';
                // }
    ...
    public function main_option_page_cb()
    {
        ...
        if ($text) {
            ...
            <div class="wrap">
                ...
                <h1>
                    <?php echo __('感谢您使用B2主题', 'b2'); ?>
                </h1>
                <?php if ($id != 1895) { ?>
                    <div id="b2-settings-opt">
                        <h2 class="title">主题安装成功</h2>
                        <p>您在我们官网的会员号是：<b class="green">
                                <?php echo $id; ?>
                            </b></p>
                        <p>您的售后截止日期为：<b class="green">
                                <?php echo $date; ?>
                            </b>
                        ...
                        <p>当前主题只适用于以下域名及其子域名，非以下域名及其子域名中无法使用，如果您在我们官网添加了新的域名，请重新下载安装主题文件。</p>
                        <div class="b2-domain-list">
                            <?php
                ...
                <div class="b2-upgrade-box" id="b2-upgrade-box">
                    <p>
                        <?php echo __('当前主题版本为：', 'b2'); ?>
                        <b class="b2-upgrade-version green">
                            <?php echo $this->get_theme_version(); ?>
                        </b>
                        <br />
                        <?php echo __('您的会员号是：', 'b2'); ?>
                        <b class="b2-upgrade-id green">
                            <?php echo $id; ?>
                        </b>
                    </p>
                    <div v-if="data && data.new_version != '<?php echo $this->get_theme_version(); ?>'" class="upgrade-info">
                        <p>最新版本：<b class="green">{{data.new_version}}</b><br />
                            发布时间：<b class="green">{{data.published_at}}</b><br />
                            更新说明：<a :href="data.info" target="_blank">更新说明</a></p>
                    </div>
                    <div v-if="data && data.new_version == '<?php echo $this->get_theme_version(); ?>'" style="margin-bottom:20px">
                        <div class="green">当前已是最新版，无需更新！</div>
                    </div>
                    <div class="b2-upgrade-box-btns"
                        v-if="!data || data.new_version == '<?php echo $this->get_theme_version(); ?>'">
                        <button class="b2-upgrade-button button empty" :disabled="loading == 'waiting'"
                            @click="checkUpdate(<?php echo $id; ?>)"><?php echo __('检查更新', 'b2'); ?></button>
                    </div>
                    <div v-if="data && data.new_version != '<?php echo $this->get_theme_version(); ?>'">
                        <button @click="upgradeTheme(<?php echo $id; ?>)" :disabled="loading == 'waiting'"
                            class="button button-primary" v-if="loading !== 'waiting'"><?php echo __('点击升级到最新版', 'b2'); ?></button>
                        <button @click="upgradeTheme(<?php echo $id; ?>)" :disabled="loading == 'waiting'"
                            style="display:flex;padding-right:20px" class="button button-primary"
                            v-else-if="loading == 'waiting'"><?php echo __('更新中，请等待完成', 'b2'); ?><b class="dots"
                                style="margin-left:5px"></b></button>
                    </div>
                    <div class="upgrade-info">
                        <p>在线升级将替换当前主题的所有文件。<span class="red"><b>如果您修改过主题源文件，请先备份好旧的主题文件，再执行升级操作。</b></span></p>
                    </div>
                </div>
                <div class="hr"></div>
                <h2 class="title">
                    <?php echo __('设置项导出', 'b2'); ?>
                </h2>
                <a class="button empty" id="b2-settings-output"
                    href="<?php echo admin_url('?page=b2_main_options&output_settings=1'); ?>">
                    <?php echo __('导出B2主题设置项', 'b2'); ?>
                </a>
                <p class="desc">
                    <?php echo sprintf(__('%s如果您的主题已经设置完毕，或者有其他重要的设置需要变更，建议及时导出.%s只限B2主题的设置项和小工具，不包括文章、分类设置、订单、卡密、邀请码等数据%s', 'b2'), '<p class="red">', '<br>', '</p>'); ?>
                </p>

                <h2 class="title">
                    <?php echo __('设置项导入', 'b2'); ?>
                </h2>
                <label class="button empty">
                    <?php echo __('导入B2主题设置项', 'b2'); ?><input type="file" accept="text/plain" id="inputsettings"
                        style="display:none" onchange="b2getFilename(event)">
                </label>
                <p class="desc">
                    <?php echo __('1、导入的设置项将会替换原有的设置项，请谨慎操作。导入的文件格式为.txt', 'b2'); ?>
                </p>
                <p class="desc">
                    <?php echo __('2、如果您已有设置好的项目，请先导出备份一下。导出的文件格式为.txt', 'b2'); ?>
                </p>

                <div class="hr"></div>

                <h2 class="title">
                    <?php echo __('请仔细阅读说明', 'b2'); ?>
                </h2>
                <p>我们已经为3000多个用户提供了优质的服务，您将也是其中之一。如果您还没有购买主题，请加QQ联系我们购买：1757558797</p>
                <p>B2主题涉及到众多敏感信息，包括支付宝，微信的账户的私密信息，实名认证信息等，请不要使用来路不明或未经授权的主题，否则造成的一切后果由您自行承担</p>
                <p>主题的加密文件中，有您登记的域名和会员号等信息，请务必不要将源码泄露给其他任何人，软件产品具有可复制性，这些信息泄露之后，将无法撤回，可能会对您造成不利的影响</p>
                <p>如果您已经购买了我们的主题，可以加入我们的QQ群：612496574</p>
                <p>域名授权以及修改地址为：<a href="https://7b2.com/check/register" target="_blank">https://7b2.com/check/register</a></p>
                <p>请不要使用我们的主题从事违反中华人民共和国法律、违反公序良俗的活动，否则我们有权拒绝服务，并配合有关单位予以处置。请详细阅读我们的协议：<a href="https://7b2.com/xieyi"
                        target="_blank">https://7b2.com/xieyi</a></p>
                <h2 class="title">售后包含的内容：</h2>
                <p>售后是指通过QQ与客服联系处理问题，售后过期不影响您对主题的使用和更新。</p>
                <p>免费三个月售后服务。售后服务时间为工作日早9点到晚5点。如果不在线可以QQ留言，12小时之内会给您答复。</p>
                <p>售后服务包括程序的使用、BUG的处理、意见建议等。不包括技术服务、网站优化、二次开发、环境配置、服务器运维等。</p>
                <h2 class="title">主题安装方法：</h2>
                <p>请根据下面的提示进行设置：</p>
                <p>1、设置服务器伪静态和固定连接：<a href="https://7b2.com/document/36878.html" target="_blank">设置伪静态和固定连接</a></p>
                <p>2、开启opcache，可以大幅提升PHP的执行效率。<span class="red">强烈建议安装</span>，安装方法：<a href="https://7b2.com/document/59371.html"
                        target="_blank">https://7b2.com/document/59371.html</a></p>
                <p>3、安装 redis 缓存：一方面可以大幅提高主题运行速度，另一方面主题一些安全功能依赖数据缓存，<span class="red">强烈建议安装</span>，安装方法：<a
                        href="https://7b2.com/document/56500.html" target="_blank">https://7b2.com/document/56500.html</a></p>
                <?php
        }
