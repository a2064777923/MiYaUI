...
class Task
{
    ...
    public static function update_task($user_id, $type)
    {
        ...
        if (strpos($type, 'task_user') === false) {
            if (!isset($_task[$type]) || (isset($_task[$type]) && (int) $_task[$type]['times'] === 0))
                return true;
        }

        $task_user = b2_task_user_arg();
        if (strpos($type, 'task_user') !== false) {
            if (!isset($task_user[$type]) || (isset($task_user[$type]) && (int) $task_user[$type]['times'] === 0))
                return true;
        }
