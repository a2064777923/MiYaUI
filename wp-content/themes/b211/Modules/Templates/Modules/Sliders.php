...
class Sliders
{
    ...
    public static function get_term_post_list($term_id, $count)
    {
        $arr = array();
        ...
        if ($term && !is_wp_error($term)) {
            ...
            $args = array(
                ...
                'posts_per_page' => $count ? $count : 5, // 获取所有文章
                'tax_query' => array(
                    array(
                        'taxonomy' => $taxonomy,
                        'field' => 'term_id',
    ...
    public static function slider_data($data)
    {
        ...
        foreach ($list_data as $k => $v) {
            ...
            $term_arr = array();
            if ($v['id'] === 'term') {
                $term_arr[] = self::get_term_post_list($v['img'], $v['title']);
            } else {
            ...
            if(!empty($term_arr)){
                foreach($term_arr as $k => $v){
                    $arg = array_merge($arg, $v);
                }
            }else{
                $arg[] = $arr;
            }
