...
class Sliders{
    ...
    public static function slider_1($data,$i){
        ...
        $slider_html ='<div class="slider-1 carousel slider-height box b2-radius '.($data['slider_width'] === '0' ? 'slider-type-width' : '').' '.($data['slider_show_title'] ? 'slider-show-title' : 'show-title-none').'" '.$slider_setting.' style="'.((int)$data['slider_width'] == 1 || $data['show_widget'] ? 'width:'.$data['width'].'px' : 'width:100%').';max-width:100%">
        '.$html.'
    </div>';
    ...
    public static function slider_7($data,$i){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $html .= '<div class="slider-1-carousel slider-height" style="width:66.66%; max-width:100%;">
                    <div class="slider-in slider-info b2-radius" style="height:0;padding-top:'.round($data['slider_height']/$data['width']*100,6).'%;max-width:100%">
                        '.b2_get_img(array('src'=>$thumb,'class'=>array('slider-img','b2-radius'))).'
                        <a class="link-block" '.(!$data['slider_new_window'] ? 'target="_blank"' : '').'  href="'.($v['link'] ? $v['link'] : 'javascript:void(0)').'"></a>
                        '.($data['slider_show_title'] ? $cat : '').'
        ...
        $slider_html ='<div class="jitheme-slider slider-1 carousel slider-height box b2-radius '.($data['slider_width'] === '0' ? 'slider-type-width' : '').' '.($data['slider_show_title'] ? 'slider-show-title' : 'show-title-none').'" '.$slider_setting.' style="'.((int)$data['slider_width'] == 1 || $data['show_widget'] ? 'width:'.$data['width'].'px' : 'width:100%').';max-width:100%">
        '.$html.'
    </div>';
    return apply_filters('b2_slider_1_html' , $slider_html);
    }
