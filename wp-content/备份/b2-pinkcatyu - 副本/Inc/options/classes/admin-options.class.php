      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo '<div class="be-help"><a href="https://www.lsenyu.com/" rel="external nofollow" target="_blank" title="主题说明"><span class="dashicons dashicons-editor-help"></span></a></div>';
            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="搜索" autocomplete="off" /></div>' : '';
            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="显示所有设置"><i class="dashicons dashicons-plus-alt"></i></div>' : '';
            echo '<div class="csf-buttons"><div class="csf-buttons-ico"><span class="dashicons dashicons-update-alt"></span></div>';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="保存设置" data-save="正在保存..." title="保存设置">';
            echo '</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? '恢复全部' : '恢复默认' ) .'" data-confirm="单击「确定」进行恢复，所有页面的配置都将丢失！">' : '';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="恢复此页" data-confirm="单击「确定」进行恢复，当前页面的配置将会丢失！">' : '';
          echo '</div>';
