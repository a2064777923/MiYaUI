...
class Posts{
    ...
    public static function post_1($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $avatar = $hove_avatar ? '<a class="post-list-meta-avatar" href="'.$v['user_link'].'">'.b2_get_img(array('src'=>$v['user_avatar'],'class'=>array('avatar','b2-radius'),'alt'=>$v['user_name'])).'<div class="jitheme_avatar_data"><span class="jitheme_avatar">'.$v['user_name'].'</span><span class="jitheme_data_d">·</span><span class="jitheme_data" >'.$date.'</span></div></a>' : '';
                                    ...
                                    <div class="card-item b-r-4 background-default overflow-hidden">
                                        <div class="onecad_app_img">
                            	            <a '.$open.' href="'.$v['link'].'" class="cardvideo-loop post-video display-block overflow-hidden post-has-video">
                                                ...
                                                <video  controls muted  class="video" autoplay="autoplay" loop="loop" data-src="'.$Onecad['view_url'].'" src="" style="display: none;"></video>
...
if ($image_count > 3) { //数字代表大于几张图显示
  ...
  $image_count_html = '';
            ...
            if(empty($v['des'])){
                $v['des']="站长很懒,没有给文章添加描述哦！有啥问题你联系站长吧";
            }
  ...
  $image_count_html = '';
            ...
            $html .= '<li class="post-3-li post-list-item">
                <div class="item-in b2-radius '.($data['post_row_count'] > 1 ? 'box' : '').'">
                    ...
                    <div class="post-info">
                        <h2><div class="post-3-jb">'.listjiaobiao($v['id']).'</div><a class="post-3-title" '.$open.' href="'.$v['link'].'">'.$v['title'].'</a></h2>
                        '.(in_array('des',$post_meta) ? 
                        '<div class="post-excerpt">
                            <p>'.$v['des'].'...</p>
                        </div>' : 
