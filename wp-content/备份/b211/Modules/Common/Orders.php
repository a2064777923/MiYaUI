...
class Orders{
    ...
    public static function build_order_action($data,$_order_id = ''){
        ...
        //order_key
        $data['order_key'] = isset($data['order_key']) ? esc_sql(str_replace(array('{{','}}'),'',b2_remove_kh($data['order_key']))) : '';

        $data['order_value'] = isset($data['order_value']) && $data['order_value'] ? urldecode($data['order_value']) : '';

        $data['order_value'] = esc_sql(str_replace(array('{{','}}'),'',b2_remove_kh($data['order_value'])));
        ...
        $data['order_content'] = isset($data['order_content']) && $data['order_content'] != '' ? esc_sql(str_replace(array('{{','}}'),'',b2_remove_kh($data['order_content']))) : '';

        $data['order_address'] = isset($data['order_address']) ? esc_sql(str_replace(array('{{','}}'),'',b2_remove_kh($data['order_address']))) : '';
    ...
    public static function get_my_orders($_user_id,$paged,$state = 'w'){
        ...
        foreach ($order as $k => $v) {
            ...
            $title = self::get_order_name($v['order_type'],$v['post_id'],$v['order_key']);
