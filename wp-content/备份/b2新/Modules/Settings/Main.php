...
class Main
{
    ...
    public function check_cg()
    {
        ...
        if ($p_v[0] >= '8.2') {
            $text = '<h2 class="red">当前版本暂未支持 php8.2，建议使用 php7.3 版本</h2> ';
        }
        ...
        if ($text) {
            ...
            <div class="wrap">
                ...
                <div class="b2-upgrade-box" id="b2-upgrade-box">
                    ...
                    <div class="b2-upgrade-box-btns" v-if="!data || data.new_version == '<?php echo $this->get_theme_version(); ?>'">
                        <button class="b2-upgrade-button button empty" :disabled="loading == 'waiting'" @click="checkUpdate(<?php echo $id; ?>)"><?php echo __('检查更新', 'b2'); ?></button>
                    </div>
                    <div v-if="data && data.new_version != '<?php echo $this->get_theme_version(); ?>'">
                        <button @click="upgradeTheme(<?php echo $id; ?>)" :disabled="loading == 'waiting'" class="button button-primary" v-if="loading !== 'waiting'"><?php echo __('点击升级到最新版', 'b2'); ?></button>
                        <button @click="upgradeTheme(<?php echo $id; ?>)" :disabled="loading == 'waiting'" style="display:flex;padding-right:20px" class="button button-primary" v-else-if="loading == 'waiting'"><?php echo __('更新中，请等待完成', 'b2'); ?><b class="dots" style="margin-left:5px"></b></button>
                    </div>
