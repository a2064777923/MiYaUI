...
$asktitle= b2_get_option('Jitheme_Ask_main','ask-title');
if(empty($asktitle)){
    $asktitle = $name;
}
?>
...
<div class="b2-single-content wrapper mg-b">
    ...
    <?php echo get_ask_cat_list($term_id) ?>
    <div id="primary-home" class="content-area wrapper ask-archive b2-radius" ref="askarchive" data-paged="<?php echo $paged; ?>" data-count="<?php echo $count; ?>" data-term="<?php echo $term_id; ?>">

        <?php
            do_action('b2_ask_archive_content_before');
        ?>
        
        <div id="jitheme_ask" class="ask-top">
            <div class="ask-slider b2-radius">
                <div class="ask-search-box">
                    <div>
                        <h1 class="ask-h1"><?php echo $asktitle;?></h1>
                    </div>
                        <div class="ask-search-input">
                        <div class="ask-search-form">
                            <input type="text" v-on:keyup.enter="search" v-model="opt.s" placeholder="<?php echo __('搜索','b2'); ?>"/><button class="text" @click="search"><i class="b2font b2-search-line "></i><?php echo __('搜索','b2'); ?></button>
                        </div>
                        <div class="ask-search-or">或</div>
                        <div class="ask-search-post">
                            <a href="<?php echo b2_get_custom_page_url('po-ask'); ?>" target="_blank" class="b2flex"><?php echo b2_get_icon('b2-edit-2-line').__('提问','b2'); ?></a>
                        </div>
                    </div>
                </div>
                <?php echo jitheme_Ask_slider() ?>
            </div>
            <div class="ask-bar b2flex">
                ...
                <div>
                    <a href="<?php echo b2_get_custom_page_url('po-ask'); ?>" target="_blank" class="b2-color button empty b2flex"><?php echo b2_get_icon('b2-edit-2-line').__('提问','b2'); ?></a>
                </div>
            </div>
        </div>
        <?php    
