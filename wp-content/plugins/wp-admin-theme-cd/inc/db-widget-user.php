    ...
    function wp_admin_list_online_users() {
        ...
        foreach( $users as $current_user ) {   
            ...
            if( empty( $getLastLogin ) ) {
                $last_user_login = esc_html__( 'N/A', 'wp-admin-theme-cd' );
            } else {
            ...
            if( empty( $getLastLogout ) ) {
                $last_user_logout = esc_html__( 'N/A', 'wp-admin-theme-cd' );
            } else {
            ...
            if( $is_logged_in != '1' ) {
                $login_status = esc_html__( 'is', 'wp-admin-theme-cd' ) . '<span class="user-status" style="background:#a5b1c2">' . esc_html__( 'logged out', 'wp-admin-theme-cd' ) . '</span>';
            } else { 
                $login_status = esc_html__( 'is', 'wp-admin-theme-cd' ) . '<span class="user-status" style="background:#20bf6b">' . esc_html__( 'logged in', 'wp-admin-theme-cd' ) . '</span>';
            }
            
            // show logged in status
            echo '<tr><td class="listing-img">' . get_avatar( $current_user->user_email, 64 ) . '</td><td><strong>' . esc_html( $current_user->display_name ) . '</strong> ' . $login_status . '<br><small>' . esc_html__( 'Last Login', 'wp-admin-theme-cd' ) . ': ' . esc_html( $last_user_login ) . '<br>' . esc_html__( 'Last Logout', 'wp-admin-theme-cd' ) . ': ' . esc_html( $last_user_logout ) . '</small></td>';
...
if( ! function_exists('wp_admin_logged_in_users_dashboard_widget') ) :
	...
	function wp_admin_logged_in_users_dashboard_widget() {
		...
		wp_add_dashboard_widget('logged_in_db_widget', esc_html__( 'User Activities', 'wp-admin-theme-cd' ), 'wp_admin_logged_in_users_dashboard_widget_content');
