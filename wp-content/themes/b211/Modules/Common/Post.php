...
class Post
{
        ...
        if ($r) {
            ...
            if (!empty($row)) {
                ...
                foreach ($row as $k => $v) {
                    ...
                    if (isset($url[1])) {
                        if (strpos($img, $url[0]) !== false)
                            return str_replace($url[0], $url[1], $img);
                    }
                }
            }
        }
    ...
    public static function b2_lazyload_action($content) {
        ...
        foreach ($imgs as $img) {
            $url = self::img_replace($img->getAttribute('src'));
            $img->setAttribute('data-src', $url);
            $img->setAttribute('src', B2_DEFAULT_IMG);
    ...
    public function publish_post($new_status, $old_status, $post)
    {
        ...
        $open_author = $open_author === '' ? false : $open_author;

        $has_email_send = get_post_meta($post->ID, 'b2_email_send', true);
            ...
            case 'post':
                ...
                if ($new_status == 'pending' && $open) {
                    ...
                    if (!$has_email_send) {
                        ...
                        update_post_meta($post->ID, 'b2_email_send', 1);
                    }
                ...
                if ($open_author && $old_status == 'pending') {
                    ...
                    if (isset($circle_data[0]['user_id'])) {
                        ...
                        Msg::send_email_to_user(
                            [
                                ...
                                'content' => '<p>' . sprintf(
                                    __('%s提交了待审%s', 'b2'),
                                    '「' . isset($user_data->display_name) ? $user_data->display_name : __('有人', 'b2') . '」',
        ...
        if ($msg_title && $old_status == 'pending' && $post->post_author != b2_get_option('normal_weixin_message', 'admin_id') && !user_can($post->post_author, 'manage_options')) {
            Wecatmp::message_success([
                ...
                "data" => [
                    "thing1" => [
                        "value" => mb_substr(sprintf(__('恭喜您！%s审核通过。', 'b2'), $msg_title), 0, 20, 'utf-8'),
                        "color" => "#173177"
                    ],
                    "thing4" => [
                        "value" => '审核通过',
    ...
    public static function save_post_qrcode($post_id, $return = false)
    {
        ...
        if ($post_type !== 'post' && $post_type !== 'shop')
            return;
     ...
     * @since 2018
     */
    public static function post_meta($post_id = 0, $opts = [])
    {
        ...
        if (isset($data['post_order']) && $opts['modified'] === 'modified') {
    ...
    public static function update_post_views($post_id)
    {
        ...
        return (int) apply_filters('b2_post_views', $views, $post_id);
    }
    ...
    public static function get_post_data($post_id, $show_author = true)
    {
        ...
        // } else {
            $views = self::update_post_views($post_id);
        // }
    ...
    public static function post_vote($type, $post_id)
    {
        ...
        if ($user_id === $post_author)
            return array('error' => __('不能给自己投票', 'b2'));
    ...
    public static function get_post_download_data($post_id, $guest = '')
    {
        ...
        if (!$can_download)
            return array('error' => __('下载未开启', 'b2'));
        ...
        if (!$download_settings || !is_array($download_settings))
            return array('error' => __('没有下载资源', 'b2'));
    ...
    public static function get_download_page_data($post_id, $index, $i, $guest = '')
    {
        ...
        $download_settings = get_post_meta($post_id, 'b2_single_post_download_group', true);
        if (!$download_settings || !isset($download_settings[$index]))
            return array('error' => __('没有找到您要下载的资源', 'b2'));
    ...
    public static function get_download_url($post_id, $index, $i)
    {
        ...
        $download_settings = get_post_meta($post_id, 'b2_single_post_download_group', true);
        if (!$download_settings || !isset($download_settings[$index]['url']))
            return array('error' => __('没有找到您要下载的资源', 'b2'));
    ...
    public static function download_file($token)
    {
        ...
        try {
            ...
            if ($down_count['count'] < 0 && $decoded->data->user_id)
                return ['error' => __('下载次数使用完毕', 'b2')];
    ...
    public static function check_current_user_can_download($post_id, $index, $user_id, $rights, $order_id = '')
    {
        ...
        foreach ($rights as $k => $v) {
            ...
            if ((isset($user_role['vip']['lv']) && $user_role['vip']['lv'] === $v['lv'])) {
                $type['vip'] = self::check_download_allow($user_id, $post_id, $index, $v, $order_id);
            }
        ...
        foreach ($rights as $k => $v) {
            ...
            if ((isset($user_role['lv']['lv']) && $user_role['lv']['lv'] === $v['lv'])) {
                $type['lv'] = self::check_download_allow($user_id, $post_id, $index, $v, $order_id);
            }
        ...
        foreach ($rights as $k => $v) {
            if ($v['lv'] === 'lv') {
                $type['alllv'] = self::check_download_allow($user_id, $post_id, $index, $v, $order_id);
            }
        }
        unset($v);
        ...
        foreach ($rights as $k => $v) {
            if ($v['lv'] === 'vip' && (isset($user_role['vip']['lv']) && strpos($user_role['vip']['lv'], 'vip') !== false)) {
                $type['allvip'] = self::check_download_allow($user_id, $post_id, $index, $v, $order_id);
            }
        }
        unset($v);
        ...
        foreach ($rights as $k => $v) {
            if ($v['lv'] === 'all' && (!$has_lv || !$has_vip)) {
                $type['all'] = self::check_download_allow($user_id, $post_id, $index, $v, $order_id);
            }
        }
    ...
    public static function insert_post($data)
    {
        ...
        if (!b2_get_option('normal_write', 'write_allow'))
            return array('error' => __('禁止投稿', 'b2'));

        //检查是否有权限
        $role = User::check_user_role($user_id, 'post');

        if (!$role && !user_can($user_id, 'manage_options') && !user_can($user_id, 'editor'))
            return array('error' => __('您没有权限发布文章', 'b2'));
        ...
        $data['content'] = isset($data['content']) ? str_replace(array('{{', '}}'), '', wp_kses_post($data['content'])) : '';
