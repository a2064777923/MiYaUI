...
class Ask
{
    ...
    public static function po_ask($data)
    {
        ...
        if (!$role && !user_can($user_id, 'manage_options') && !user_can($user_id, 'editor'))
            return array('error' => __('您没有权限提问', 'b2'));
        ...
        $data['title'] = isset($data['title']) ? apply_filters('b2_sanitize_data', $data['title']) : '';
        $data['content'] = isset($data['content']) ? apply_filters('b2_sanitize_data', $data['content']) : '';
        ...
        if ($post_id) {
            ...
            if (!empty($data['inv'])) {
                ...
                $data['inv'] = apply_filters('b2_sanitize_data', $data['inv']);
            ...
            if ($data['reward']) {
                ...
                self::sub_reward([
                    ...
                    'post_id' => $post_id,
                    'reward' => $data['reward'],
                    'rewardType' => apply_filters('b2_sanitize_data', $data['rewardType']),
                    'money' => $data['money'],
    ...
    public static function back_money($post_id, $msg)
    {
        ...
        if (!isset($old['rewardType']) || !isset($old['money']))
            return;
        $gold_type = '';
    ...
    public static function check_money_back($post_id)
    {
        ...
        if (get_post_meta($post_id, 'b2_ask_has_reward', true))
            return;
    ...
    public static function po_ask_answer($data)
    {
        ...
        if (!$data['parent_id'])
            return ['error' => __('非法操作', 'b2')];

        if (get_post_type($data['parent_id']) !== 'ask')
            return ['error' => __('非法操作', 'b2')];
        ...
        if (!$role && !user_can($user_id, 'manage_options') && !user_can($user_id, 'editor'))
            return array('error' => __('您没有权限回答问题', 'b2'));
    ...
    public static function answer_right($data)
    {
        ...
        if (get_post_meta($parent, 'b2_ask_has_reward', true))
            return true;
        if (isset($pay_type['reward']) && $pay_type['reward']) {

            if (!self::ask_end_time($parent))
                return false;

            if (!isset($pay_type['rewardType']) || !isset($pay_type['money']))
            ...
            if ($pay_type['money'] <= 0 || $pay_type['money'] > 9999999)
                return ['error' => __('参数错误', 'b2')];
