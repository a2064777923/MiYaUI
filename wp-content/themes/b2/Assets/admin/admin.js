...
var fontCdn = jQuery('#fonts_cdn').find(':selected').val();
if (fontCdn == '1') {
	jQuery('.cmb2-id-fonts-link').show();
} else {
	jQuery('.cmb2-id-fonts-link').hide();
}

jQuery('#fonts_cdn').on('change', function () {
	var val = jQuery(this).find(':selected').val();
	if (val == '1') {
		jQuery('.cmb2-id-fonts-link').show();
	} else {
...
var b2upgrade = new Vue({
	...
	methods: {
		checkUpdate(id) {
			if (this.loading == 'waiting') return
