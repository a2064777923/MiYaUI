...
class Links
{
    ...
    public static function submit_link($data)
    {
        ...
        if (!$user_id)
            return array('error' => __('请登录后操作', 'b2'));

        $open = b2_get_option('links_submit', 'link_submit_open');
        if($open == 0 && !user_can($user_id, 'manage_options')){
            return array('error' => __('不允许申请入驻', 'b2'));
        }
        ...
        foreach ($data as $key => $value) {
            if (!isset($arg[$key]) || $value == '')
                return array('error' => __('请填写完整的参数', 'b2'));
        ...
        if (is_array($allow_cats) && !empty($allow_cats)) {
            if (!in_array((string) $data['link_category'], $allow_cats))
                return array('error' => __('不允许入驻此分类', 'b2'));
        }

        $data['link_url'] = esc_url($data['link_url']);

        //检查是否有待审
        if (self::has_pending($user_id) && !user_can($user_id, 'manage_options'))
            return array('error' => __('您已经提交过，审核通过后方可再次提交', 'b2'));

        if (!self::or_url($data['link_url']))
            return array('error' => __('您提交的不是网址', 'b2'));
        if (!self::or_url($data['link_image']))
            return array('error' => __('图标格式错误', 'b2'));

        if (!attachment_url_to_postid($data['link_image']))
    ...
    public static function link_vote($link_id)
    {
        ...
        $user_id = b2_get_current_user_id();
        if (!$user_id)
            return array('error' => __('请登陆后操作', 'b2'));
