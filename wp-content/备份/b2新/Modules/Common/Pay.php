...
class Pay
{
    ...
    public static function pay_type($type)
    {
        ...
        if (!in_array($type, $arg))
            return array('error' => __('支付类型错误！', 'b2'));

        if (strpos($type, 'alipay') !== false) {

            $alipay_type = b2_get_option('normal_pay', 'alipay');
            if (!$alipay_type)
                return array('error' => __('未启用支付宝', 'b2'));
            return array(
                'pick' => 'alipay',
                'type' => $alipay_type
            );
            ...
            if (!$wecatpay_type)
                return array('error' => __('未启用微信支付', 'b2'));
            return array(
                'pick' => 'wecatpay',
                'type' => $wecatpay_type
            );
    ...
    public static function credit_pay($order_id)
    {
        ...
        if (empty($data))
            return array('error' => __('支付信息错误', 'b2'));

        if ($data['order_state'] !== 'w')
            return array('error' => __('支付信息错误', 'b2'));
    ...
    public static function pay_notify($method, $post)
    {
        ...
        if (!$order_id)
            return array('error' => __('订单获取失败', 'b2'));
        ...
        if ($type === 'xunhu')
            return self::xunhu_notify($post, $order);
    ...
    public static function xorpay_notify($method, $post)
    {
        ...
        if (!isset($post['aoid']) || !isset($post['sign']) || !isset($post['order_id']) || !isset($post['pay_price']) || !isset($post['pay_time']))
    ...
    public static function paypal($data)
    {
        ...
        if (!is_string($res))
            return ['error' => isset($res['L_LONGMESSAGE0']) ? $res['L_LONGMESSAGE0'] : __('paypal配置错误，请检查', 'b2')];
    ...
    public static function xunhu_action($data, $settings)
    {
        ...
        if ($is_mobile && $settings['payment'] == 'wechat') {
            $settings['type'] = 'WAP';
    ...
    public static function baidu($data)
    {
        ...
        if (!isset($opt['pay_baidu_dealId']))
            return array('error' => __('请先前往移动端插件中配置百度支付', 'b2'));
