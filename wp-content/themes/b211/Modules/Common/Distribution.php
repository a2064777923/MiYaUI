...
class Distribution
{
    ...
    public static function distribution_action($data)
    {
        ...
        if (!in_array($data['order_type'], $allow))
            return $data;
    ...
    public static function user_can_distribution($user_id)
    {
        ...
        if ((int) b2_get_option('distribution_main', 'distribution_open') === 0)
            return false;
    ...
    public static function get_my_distribution_data($user_id = 0)
    {
        ...
        if (!self::user_can_distribution($current_user_id))
            return array(
                'total_money' => '****',
                'ref' => '*********'
            );
    ...
    public static function get_my_distribution_orders($user_id = 0, $paged = 1)
    {
        ...
        $data = $wpdb->get_results(
            $wpdb->prepare(
                ...
                $current_user_id,
                'post_distribution_lv1',
                'post_distribution_lv2',
                'post_distribution_lv3',
                $offset,
                $number
            )
            ,
