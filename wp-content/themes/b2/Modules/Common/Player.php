...
class Player {
    ...
    public function player_shortcode_mp($atts = array(), $content = '' ){
        ...
        if($allow){
            ...
            return '<video width="100%" class="b2videobox" id="myVideo" oncontextmenu="return false;" poster="'.$poster.'" src="'.$url.'" controls></video>';
        }else{
    ...
    public static function check_video_allow($post_id,$order_id = ''){
        ...
        foreach($videos as $k => $v){
            ...
            $v['title'] = str_replace($h2.'|','',isset($v['title']) ? $v['title'] : '');
