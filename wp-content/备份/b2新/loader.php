    ...
    class B2
    {
        ...
        public function load_library()
        {
            ...
            try {
                ...
                if ($ver < '3.1') {
                    if (!$this->is_admin) {
                        if (current_user_can('administrator')) {
                            wp_die('<h2>' . __('升级扩展', 'b2') . '</h2><p>' . sprintf(__('请前往%s主题激活%s页面根据提示操作', 'b2'), '<a href="' . admin_url('/admin.php?page=b2_main_options') . '" target="_blank">', '</a>') . '</p>');
                        } else {
                ...
                if (!$this->is_admin) {
                    if (current_user_can('administrator')) {
                        wp_die('<h2>' . __('安装扩展', 'b2') . '</h2><p>' . sprintf(__('请前往%s安装扩展%s页面安装扩展', 'b2'), '<a href="' . admin_url('/admin.php?page=b2_main_options') . '" target="_blank">', '</a>') . '</p>');
                    } else {
                        wp_die('<h2>' . __('系统维护中.....', 'b2') . '</h2><p>' . __('如果您是管理员，请登陆后台操作', 'b2') . '</p>');
                    }
                }
