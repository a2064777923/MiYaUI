...
class Orders{
    ...
    public static function build_order($data){
        ...
        if($dark_room) return array('error'=>__('小黑屋中禁止交易','b2'));

        $can_guset_pay = apply_filters('b2_can_guest_buy', $data);

        if(!$user_id && !(int)$can_guset_pay) return array('error'=>__('请先登录','b2'));

        $allow_cz = b2_get_option('normal_gold','gold_cz');
        if(!$allow_cz && $data['order_type'] == 'cz') return ['error'=>__('充值功能已经关闭')];

        $allow_cg = b2_get_option('normal_gold','gold_cg');
        if(!$allow_cg && $data['order_type'] == 'cg') return ['error'=>__('积分购买功能已经关闭')];
        ...
        $data = apply_filters('b2_check_order_value', $data);
    ...
    public static function build_order_action($data,$_order_id = ''){
        ...
        if($data['order_type'] == 'vip'){
            ...
            if(!isset($user_data->display_name)) return ['error'=>__('请先登录','b2')];
        }
        ...
        //标题
        if(isset($data['title'])){
            $data['title'] = b2_get_des(0,30,$data['title']);
        }
        ...
        //order_key
        $data['order_key'] = isset($data['order_key']) ? str_replace(array('{{','}}'),'',b2_remove_kh($data['order_key'])) : '';

        // $data['order_value'] = isset($data['order_value']) && $data['order_value'] ? $data['order_value']: '';


        // return ['error'=>$data['order_value']];

        $data['order_value'] = str_replace(array('{{','}}'),'',sanitize_text_field($data['order_value']));
        ...
        //order_content
        $data['order_content'] = isset($data['order_content']) && $data['order_content'] != '' ? str_replace(array('{{','}}'),'',sanitize_text_field($data['order_content'])) : '';

        $data['order_address'] = isset($data['order_address']) ? str_replace(array('{{','}}'),'',sanitize_text_field($data['order_address'])) : '';
    ...
    public static function callback_d($data){
        ...
        if(!isset($user_data->display_name)) return;
    ...
    public static function get_order_info($order_id){
        ...
        $order = $wpdb->get_row(
            $wpdb->prepare("SELECT * FROM $table_name WHERE order_id = %s AND user_id = %d", $order_id, $user_id),
            ARRAY_A
        );

        if(!$order){
            return array('error'=>__('订单不存在','b2'));
        }

        return self::order_item($order,$user_id);
    }
    ...
    public static function get_my_orders($_user_id,$paged,$state = 'w'){
        ...
        foreach ($order as $k => $v) {
            ...
            $data[] = self::order_item($v,$user_id);
        }
    ...
    public static function order_states(){
       return array(
            'w'=>__('等待付款','b2'),
            'f'=>__('已付款未发货','b2'),
            'c'=>__('已发货','b2'),
    ...
    public static function order_item($order,$user_id){
            $order_type = b2_order_type($order['post_id']);

            $type = isset($order_type[$order['order_type']]) ? $order_type[$order['order_type']] : __('未知订单类型','b2');

            $title = self::get_order_name($order['order_type'],$order['post_id'],$order['order_key']);

            $track = maybe_unserialize($order['tracking_number']);
            $tk = b2_express_types();
            $tk = isset($track['type']) ? $tk[$track['type']] : '';
            $nb = isset($track['number']) ? $track['number'] : '';


            $_type = get_post_meta($order['post_id'],'zrz_shop_type',true);

            $shop_multi = get_post_meta($order['post_id'],'zrz_shop_multi',true);

            $desc = [];

            if($_type == 'normal' && $shop_multi == 1){

                $json = json_decode($order['order_value'],true );
            ...
            $order_list = [];
            //计算订单过期时间
            if($order['order_state'] === 'w' && $order['order_type'] === 'g'){
                //$v['order_date'] 加 60分钟
                ...
                $order_list = json_decode(stripslashes($order['order_value']),true);
                $order_list = self::get_gorder_data($order_list['products'],$user_id);
                
            }

            return apply_filters( 'b2_get_order_item', array(
                'id'=>$order['id'],
                'desc'=>$desc,
                ...
                'pass_days'=>$pass_days,
                'order_key'=>$order['order_key'] == 'cq' ? 'cq' : '',
                'order_state'=>isset(self::order_states()[$order['order_state']]) ? self::order_states()[$order['order_state']] : 'w',
                '_order_state'=>$order['order_state'],
                'tracking_number'=>isset($order['tracking_number']) ? array('type'=>$tk,'number'=>$nb,'com'=>isset($track['type']) ? $track['type'] : '') : '',
                'money_type'=>$order['money_type'],
                'order_type'=>$type,
                '_order_type'=>$order['order_type'],
                'order_commodity'=>$order['order_commodity'],
                'pay_type'=>$pay_type,
                'thumb'=>$title['img'],
    ...
    public static function get_order_name($type,$id,$order_key = 0){
        ...
        if($type === 'cz'){
            ...
            $name = array(
                'name'=>get_the_title($order_key),
                'link'=>get_permalink($order_key)
            );
            $img = b2_get_thumb(array('thumb'=>Post::get_post_thumb($order_key),'width'=>100,'height'=>100));
        }else{
            $name = array(
