...
class Login
{
    ...
    public static function delete_user($arg)
    {
        ...
        if (user_can($user_id, 'administrator'))
            return array('error' => __('管理员无法注销', 'b2'));

        if (!isset($arg['password']))
            return array('error' => __('请输入密码', 'b2'));
    ...
    public static function juhe_social_login($type)
    {
        ...
        if (!in_array($type, $social_types))
            return ['error' => __('参数错误', 'b2')];
    ...
    public static function check_login_name($current_user_id)
    {
        ...
        if ($check_type === 'telandemail') {
            if ((empty($user_data->data->user_email)) && !self::is_phone($user_data->data->user_login))
                return 'telandemail';
            return false;
        }

        if ($check_type === 'text' || $check_type === 'luo') {
            if (strpos($user_data->data->user_login, 'user' . $current_user_id) !== false)
                return 'login';
            return false;
        }
    }
    ...
    public static function bind_user_login($data)
    {
        ...
        if (!$type)
            return array('error' => __('您不需要此操作', 'b2'));
    ...
    public static function send_code($request)
    {
        ...
        if (wp_using_ext_object_cache() && $request['token']) {
            ...
            if ($isset_token)
                return array('error' => __('请不要重复提交', 'b2'));
        }
    ...
    public static function regeister($request)
    {
        ...
        $limit = apply_filters('b2_request_limit', '', 10, 600);
        
        if (!$limit) {
            return ['error' => __('操作频次过高', 'b2')];
        }
        ...
        //检查昵称
        $nickname = self::check_nickname($request['nickname'], true);
    ...
    public static function regeister_action($data, $check_type, $check_invitation)
    {
        ...
        wp_cache_set('b2_register_limit_' . md5($ip), ($has_register_count + 1), 'b2_register_limit', HOUR_IN_SECONDS);
    ...
    public static function check_email($email)
    {
        ...
        //如果邮箱在黑名单中
        if (in_array($_email, (array)$black_list)) {
            return array('error' => sprintf(__('不能使用%s的邮箱在本站注册', 'b2'), $_email));
        }

        //如果在白名单中
        if ($white_list && in_array($_email, (array)$white_list)) {
            return $email;
        }
    ...
    public static function check_verify_code($request)
    {
        ...
        if (!isset($request['smsToken']) || !isset($request['code']))
            return array('error' => __('请输入验证码', 'b2'));
        if (!$request['smsToken'] || !$request['code'])
            return array('error' => __('请输入验证码', 'b2'));
        ...
        try {
            ...
            if (!isset($decoded->data->username) || $decoded->data->username != $request['username']) {
                return array('error' => __('验证失败，请重新提交', 'b2'));
            }
    ...
    {
        if ($username == '')
            return array('error' => __('请输入邮箱或手机号码', 'b2'));
        ...
        if ($check_type == 'text' || $check_type == 'luo') {
            ...
            if (strlen($username) < 6) {
                return array('error' => __('登录名必须大于6位', 'b2'));
            }
        }
    ...
    public static function check_nickname($nickname, $check = false)
    {

        $nickname = wp_strip_all_tags($nickname);

        $nickname = self::match_chinese($nickname);

        $nickname = apply_filters('b2_check_nickname', $nickname);
        ...
        if (self::strLength($nickname) > 8)
            return array('error' => __('昵称太长了！最多8个字符', 'b2'));
        ...
        if ($check) {
            if ($result)
                return array('error' => __('该昵称已被注册', 'b2'));
        }
    ...
    public static function rest_pass($request)
    {
        ...
        if ($request['confirmPassword'] == '')
            return array('error' => __('请输入确认密码', 'b2'));
