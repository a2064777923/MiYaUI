...
class User
{
    ...
    public function init()
    {
        ...
        add_filter('get_avatar_data', array($this, 'get_avatar_cus'), 10, 2);
    ...
    public function get_avatar_cus($args, $id_or_email)
    {
        ...
        if ($id_or_email === 'email@example.com')
            return $args;
    ...
    public static function get_user_normal_data($user_id)
    {
        ...
        if (!$user_id)
            return array('error' => __('无法获取用户信息', 'b2'));

        if (isset($GLOBALS['b2_user_normal_data_' . $user_id]))
            return $GLOBALS['b2_user_normal_data_' . $user_id];
    ...
    public static function get_user_public_data($user_id, $private = false)
    {
        ...
        if($show_desc) {
            $user_public_data['desc'] = isset($data->description) ? esc_attr($data->description) : '';
        }
        ...
        wp_cache_set('b2_user_' . $user_id, $data, 'b2_user_data', 600);
    ...
    {
        if (!$url || !$id || !$user_id)
            return array('error' => __('参数不全', 'b2'));

        $current_user_id = b2_get_current_user_id();

        if ((int) $user_id !== $current_user_id && !user_can($current_user_id, 'administrator'))
            return array('error' => __('非法操作', 'b2'));

        $open = get_user_meta($user_id, 'zrz_open', true);
        $open = !empty($open) ? $open : array();

        $open['cover']['key'] = esc_url(apply_filters('b2_sanitize_data',$url));
    ...
    public static function put_dark_room($user_id, $days = 0, $why = '')
    {
        ...
        $msg = sprintf(__('您被关进了小黑屋%s天', 'b2'), $days);
    ...
    public static function get_lv_icon($lv, $return = false)
    {
        ...
        if (isset($roles[$lv])) {
            ...
            if (strpos($lv, 'vip') !== false) {
                ...
                $icon = apply_filters('b2_get_vip_icon', '<span class="lv-icon user-'.preg_replace("[\d]", '', $lv).' b2-'.$lv.'"><img src="'.B2_CHILD_URI.'/pic/'.$lv.'.png" style="height: 20px;" title="'.$name.'"></span>',$lv,$name);
            }
        }
    ...
    public static function get_following($user_id, $paged, $number)
    {
        ...
        if ((int) $number > 80)
            return array('error' => __('请求数量过多', 'b2'));
        if ((int) $paged < 0)
            return array('error' => __('请求格式错误', 'b2'));
    ...
    public static function user_follow_action($user_id)
    {
        ...
        if ($current_user_id == $user_id)
            return array('error' => __('不能关注自己', 'b2'));
    ...
    public static function user_favorites($post_id)
    {
        ...
        if (get_post_status($post_id) !== 'publish')
            return ['error' => __('该文章无法收藏', 'b2')];

        $current_user_id = b2_get_current_user_id();

        if (!$current_user_id)
            return array('error' => __('请登录', 'b2'));
        if (!$post_id)
            return array('error' => __('参数不全', 'b2'));
    ...
    public static function get_default_address($user_id, $key = '')
    {
        ...
        if (!isset($userData['address']))
            return array('error' => __('收货地址不存在', 'b2'));
    ...
    public static function get_author_settings($user_id)
    {
        ...
        if (!$current_user_id)
            return array('error' => __('您没有权限进行此项操作', 'b2'));
        ...
        $data = array(
            ...
            'email' => $email && strpos( B2_HOME_URI,  $_email) === false ? $email : '',
    ...
    public static function un_build($type, $user_id)
    {
        ...
        if ($type == 'weixin') {
            ...
            if (!$uid)
                return array('error' => __('您未绑定此账户，无需解绑', 'b2'));
        }
        ...
        if (!is_numeric($user_info->data->user_login) && strpos($user_info->data->user_email, (string) $current_user_id) !== false && !user_can($current_user_id, 'administrator')) {
            ...
            if ($days <= 2)
                return array('error' => __('系统原因，暂时无法解绑', 'b2'));
        }
    ...
    public static function save_nick_name($name, $user_id)
    {
        ...
        $res = Login::check_nickname($name, true);
        if (isset($res['error'])) {
            return $res;
        }
    ...
    public static function save_url($url, $user_id)
    {
        ...
        if (empty($url))
            return array('error' => __('网址不可为空', 'b2'));

        $url = str_replace(array('{{', '}}'), '', esc_url($url));

        $url = b2_remove_kh($url);
    ...
    public static function save_desc($desc, $user_id)
    {
        ...
        $desc = apply_filters('b2_sanitize_data', $desc);
        ...
        if (empty($desc))
            return array('error' => __('描述不可为空', 'b2'));
    ...
    public static function save_address($data)
    {
        ...
        if (!$address || !$name || !$phone || !$key)
            return array('error' => __('请填写完整的地址信息', 'b2'));

        if (!Login::is_phone($phone))
            return array('error' => __('您填写的不是手机号码', 'b2'));

        $userData = get_user_meta($user_id, 'zrz_user_custom_data', true);
        $userData = isset($userData['address']) && is_array($userData['address']) ? $userData : array();

        $userData['address'][esc_attr($key)] = array(
            'address' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $address))),
            'name' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $name))),
            'phone' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $phone))),
            'province' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $province))),
            'city' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $city))),
            'county' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $county))),
            'regionCode' => str_replace(array('{{', '}}'), '', b2_remove_kh(apply_filters('b2_sanitize_data', $regionCode))),
        );
    ...
    public static function save_username($data)
    {
        ...
        if ((int) $data['user_id'] !== $current_user_id && !user_can($current_user_id, 'administrator'))
            return array('error' => __('非法操作', 'b2'));
    ...
    //修改密码
    public static function edit_pass($passenyu, $repassenyu, $user_id)
    {
        $current_user_id = b2_get_current_user_id();
        ...
        if ($passenyu !== $repassenyu)
            return array('error' => __('请重复输入两次相同的密码！', 'b2'));

        if (strlen($passenyu) < 6) {
            return array('error' => __('密码必须大于6位', 'b2'));
        }

        wp_set_passenyu($passenyu, $user_id);
    ...
    public static function search_users($key)
    {
        ...
        if (!$key)
            return array('error' => __('用户名不可为空', 'b2'));
    ...
    public static function cash_out($money)
    {
        ...
        if ((int) b2_get_option('normal_gold', 'gold_tx') === 0)
            return array('error' => __('禁止提现', 'b2'));
    ...
    public static function mission_task($user_id)
    {
        ...
        //去掉空白
        $mission_task = trim($mission_task);
        
        if ((int) $mission_task == 0)
            return;

        $mission_task = explode('|', $mission_task);

        if (!isset($mission_task[1]) || (int) $mission_task[0] == 0 || (int) $mission_task[1] == 0)
            return;
    ...
    public static function get_mission_user_list($type, $count, $paged)
    {
        ...
        if ((int) $paged < 0)
            return array('error' => __('数据格式错误', 'b2'));

        $offset = ($paged - 1) * $count;

        if ((int) $count > 50)
            return array('error' => __('请求数据数量过多', 'b2'));
    ...
    public static function get_user_custom_data($user_id)
    {
        ...
        if ($user_id) {
            if (isset($GLOBALS['b2_user_custom_data_' . $user_id]))
                return $GLOBALS['b2_user_custom_data_' . $user_id];
    ...
    public static function get_current_user_attachments($type, $paged)
    {
        ...
        if (!$type)
            return array('error' => __('文件类型错误', 'b2'));
    ...
    public static function check_user_role($user_id, $type)
    {
        ...
        if ($lv && isset($roles[$lv])) {
            ...
            if (is_array($role) && in_array($type, $role))
                return true;
        }
    ...
    public static function get_top_data($data)
    {
        ...
        if (!isset($data['number']) || (int) $data['number'] > 30 || (int) $data['number'] <= 0)
            $data['number'] = 20;
        ...
        if (!empty($user_query->results)) {
            foreach ($user_query->results as $user) {
                ...
                if (!isset($_data['desc']) || $_data['desc'] === '') {
    ...
    public static function shield_author($request)
    {
        ...
        if ($request['type'] !== 'comment' && $request['type'] !== 'post')
            return array('error' => __('参数错误', 'b2'));
        ...
        if ($user_id == $author_id)
            return array('error' => __('不能屏蔽自己', 'b2'));
        ...
        if ($user_shields) {
            if (in_array($author_id, $user_shields))
                return array('error' => __('您已经屏蔽了此用户', 'b2'));
    ...
    public static function check_post($user_id)
    {
        ...
        if (!$user_id)
            return array('error' => __('登录后操作', 'b2'));
    ...
    public static function save_check_post_count($user_id)
    {
        $has_post_count = (int) wp_cache_get('b2_post_limit_' . $user_id, 'b2_post_limit');
        if ($has_post_count === 0) {
            wp_cache_set('b2_post_limit_' . $user_id, 1, 'b2_post_limit', 600);
        } else {
