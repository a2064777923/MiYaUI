...
class Shop
{
    ...
    public static function get_shop_item_data($post_id, $user_id, $return = array(), $index = '')
    {
        ...
        if ($status !== 'publish')
            return array('error' => __('不存在此商品', 'b2'));
    ...
    public static function user_can_buy($post_id, $user_id, $type, $limit = [])
    {
        ...
        foreach ($roles as $k => $v) {
            ...
            if (strpos($v, 'lv') !== false) {
                ...
                $lv_data = isset($lvs[preg_replace('/\D/s', '', $v)]) ? $lvs[preg_replace('/\D/s', '', $v)] : [];
                if(empty($lv_data)) continue;

                $_lvs[] = [
                    'name' => isset($lv_data['name']) ? $lv_data['name'] : '',
                    'color' => '',
                    'lv' => $v,
                    'class' => 'lv'
                ];
            } else {
                $key = preg_replace('/\D/s', '', $v);
                $lv_data = isset($vips[$key]) ? $vips[$key] : [];
    ...
    public static function shop_lottery_action($post_id, $address)
    {
        ...
        //检查是否是抽奖类型
        if (get_post_meta($post_id, 'zrz_shop_type', true) !== 'lottery')
            return array('error' => __('商品类型错误', 'b2'));

        if ($check['allow'] === false)
            return array('error' => __('抽奖', 'b2') . $check['text']);
    ...
    public static function shop_get_express_data($com, $order_id, $address = '')
    {
        ...
        $request = wp_remote_post('https://ali-deliver.showapi.com/showapi_expInfo', $args);
    ...
    public static function set_my_carts($data)
    {
        ...
        if ($post_type != 'shop')
            return array('error' => __('这个不是商品', 'b2'));
        ...
        if ((int) $data['count'] > $total)
            return array('error' => __('库存不足', 'b2'));
