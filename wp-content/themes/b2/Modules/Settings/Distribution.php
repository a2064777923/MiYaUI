...
class Distribution{
    ...
    public function distribution_settings(){
        ...
        $distribution->add_field(array(
            ...
            'before_row'=>'<p>'.sprintf(__('1、您可以在文章、商品、问答、%s中设置是否允许该商品进行提成，只有这些商品开启了提成功能，推广系统才会进行提成操作','b2'),$circle_name).'</p>
            <p>'.__('2、拥有推广权限的用户会拥有一个推广连接，通过这个推广连接购买后，会按照比例给推广人提成','b2').'</p>
            <p>'.sprintf(__('3、%s充值、打赏等不会触发提成','b2'),B2_MONEY_NAME).'</p>
            <p>'.__('4、自己使用自己的推广连接购买不进行提成','b2').'</p>
            <p>'.__('6、推广关系在第一次使用推广连接的时候就会固化','b2').'</p>
            <p class="red">'.__('7、请根据本国或地区的相关法律规定来确定推广层级，请勿触犯所在国家或地区的法律。中华人民共和国境内规定包括本人在内，推广层级不能超过三级，所以强烈推荐中国用户只启用一级。','b2').'</p>'
        ));
        ...
        $distribution->add_field(array(
            ...
            'desc'=>__('文章中可以单独设置是否允许提成，不设置则默认使用此处的设置','b2')
        ));

        $distribution->add_field(array(
            'name'    => __( '商品出售默认是否允许提成？', 'b2' ),
            ...
            'desc'=>__('商品中可以单独设置是否允许提成，不设置则默认使用此处的设置','b2')
        ));

        $distribution->add_field(array(
            'name'    => __( 'VIP购买是否允许提成？', 'b2' ),
            'id'=>'distribution_vip',
            'type' => 'select',
            'options'=>array(
                1=>__('允许提成','b2'),
