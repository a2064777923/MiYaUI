...
class Posts{
                                    ...
                                    <div class="card-item b-r-4 background-default overflow-hidden">
                                        <div class="onecad_app_img">
                            	            <a '.$open.' href="'.$v['link'].'" class="cardvideo-loop post-video display-block overflow-hidden post-has-video">
                                                ...
                                                <video class="video" muted="muted"  autoplay="autoplay" loop="loop" data-src="'.$Onecad['view_url'].'" src="" style="display: none;"></video>
            ...
            $html .= '<li class="post-list-item item-'.$post_style.'" id="item-'.$v['id'].'">
                <div class="item-in box b2-radius">
                    ...
                    <div class="post-info jitheme-post-info">
                        ...
                        <h2><a '.$open.' href="'.$v['link'].'">'.jitheme_post_time_ls($v['id']).$v['title'].'</a></h2>
    ...
    public static function post_2($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $links_ysa_a1 =get_post_meta($v['id'],'b2_single_post_video_group',true);
                if(is_array($links_ysa_a1)){
                    foreach ($links_ysa_a1 as $k => $Onecad) {
                        $oneship ='<div class="post-module-thumb jitheme-radius" style="padding-top:'.($v['thumb_ratio']*100).'%">
                                <div class="width-1-4" id="Onecad_vido" >
                                    <div class="card-item b-r-4 background-default overflow-hidden">
                                        <div class="onecad_app_img">
                            	            <a '.$open.' href="'.$v['link'].'" class="cardvideo-loop post-video display-block overflow-hidden post-has-video">
                                                <span class="play-icon"></span>
                                                <video class="video" muted="muted" autoplay="autoplay" loop="loop" data-src="'.$Onecad['view_url'].'" src="" style="display: none;"></video>
                                                '.b2_get_img(array('src'=>$thumb,'class'=>array('width-1-1 show-image'),'alt'=>$v['title'])).'
                                            </a>
                        	            </div>
                                    </div>
                ...
                $mp3_audio =get_post_meta($v['id'],'onecad_audio_url',true);
                $oneship ='<div class="post-module-thumb" style="padding-top:'.($v['thumb_ratio']*100).'%">
                    <div class="card-item b-r-4 background-default overflow-hidden">
                ...
                $oneship ='<div class="post-module-thumb jitheme-radius" style="padding-top:'.($v['thumb_ratio']*100).'%">
                        <a '.$open.' href="'.$v['link'].'" rel="nofollow" class="thumb-link">'.b2_get_img(array('src'=>$thumb,'class'=>array('post-thumb','b2-radius'),'alt'=>$v['title'])).self::get_post_icon($v['id']).'</a>
                        '.($v['status'] === 'pending' ? '<span class="post-status">'.__('审核中...','b2').'</span>' : ($v['status'] === 'draft' ? '<span class="post-status">'.__('草稿','b2').'</span>' : '')).'
                        '.$pay_type.'
                        '.$meta_html.'
                        <div class="jitheme_cat_jb">'.listjiaobiao($v['id']).$ji_meta_html.'</div>
            ...
            //文章列表
            $html .= '<li class="grid-item post-list-item  item-'.$post_style.'" id="item-'.$v['id'].'">
                <div class="item-in box b2-radius">
                    '.$oneship.'
                    '.Psotjianb($v['id']).'
                    <div class="post-info jitheme-post-info">
    ...
    public static function post_3($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $links_ysa_a1 =get_post_meta($v['id'],'b2_single_post_video_group',true);
                if(is_array($links_ysa_a1)){
                    foreach ($links_ysa_a1 as $k => $Onecad) {
                        $oneship ='<div class="post-module-thumb b2-radius" style="padding-top:'.$size['ratio'].'%">
                                <div class="width-1-4 b2-radius" id="Onecad_vido" >
                                    <div class="card-item b-r-4 background-default overflow-hidden">
                                        <div class="onecad_app_img">
                            	            <a '.$open.' href="'.$v['link'].'" class="cardvideo-loop post-video display-block overflow-hidden post-has-video">
                                                <span class="play-icon"></span>
            ...
            $html .= '<li class="post-3-li post-list-item">
                <div class="item-in b2-radius '.($data['post_row_count'] > 1 ? 'box' : '').'">
                    ...
                    <div class="post-info">
                        <h2><div class="post-3-jb">'.listjiaobiao($v['id']).'</div><a class="post-3-title" '.$open.' href="'.$v['link'].'">'.jitheme_post_time_ls($v['id']).$v['title'].'</a></h2>
                        '.(in_array('des',$post_meta) ? 
                        '<div class="post-excerpt">
                            <p>'.jitheme_desc_unll($v['des']).'</p>
                        </div>' : 
                        '').'
                        '.$meta_html.'
    ...
    public static function post_5($data,$i,$return = false){
        ...
        foreach ($post_data as $k => $v) {
            ...
            $html .= '<li class="grid-item post-list-item" id="item-'.$v['id'].'">
                <div class="item-in">
                    <div class="post-info">
                        <h2><a '.$open.' href="'.$v['link'].'">'.jitheme_post_time_ls($v['id']).$v['title'].'</a></h2>'.$date.'
                    </div>
                        ...
                        $oneship ='<div class="post-module-thumb" style="padding-top:'.$size['ratio'].'%">
                                <div class="width-1-4 " id="Onecad_vido" >
                                    <div class="card-item b-r-4 background-default overflow-hidden">
                                        <div class="onecad_app_img">
                            	            <a '.$open.' href="'.$v['link'].'" class="cardvideo-loop post-video display-block overflow-hidden post-has-video">
                                                <span><i class="b2font b2-play-circle-line"></i></span>
                                                <video class="video" muted="muted" autoplay="autoplay" loop="loop" data-src="'.$Onecad['view_url'].'" src="" style="display: none;"></video>
                                                '.b2_get_img(array('src'=>$thumb,'class'=>array('width-1-1 show-image'),'alt'=>$v['title'])).'
                                            </a>
                        	            </div>
                                ...
                                <div class="jitheme_cat_jb">'.listjiaobiao($v['id']).'</div>
                                </div>';
                    break;
                    }
                }
    ...
    public static function get_modules_title($data){
        ...
        if($title && isset($data['title'])){
           ...
           $html .= '<div class="Onecad_title"><div>'.$data['title'].'</div>';
