...
class Ask extends \WP_Widget{
	...
	public static function get_widget( $atts ) {
			...
			<div id="jitheme_ask_b2" class="ask-widget-list">
				...
				<div v-if="data !== \'\'" v-cloak>
					...
					<ul class="ask-widget-ul" v-else>
                        <li class="ask-widget-item" v-for="(item,i) in data.data">
                            <a :href="item.link" target="_blank" class="link-block"></a>
                                <div class="ask-widget-title">
                                    <div class="item-index"><i :class="`Jifont-fire-${i + 1}`" class="Jifont"></i></div>
                                <div>
                                <div class="jitheme_title" v-text="item.title"></div>
                                    <div class="b2flex ask-widget-meta">
                                        <div><span v-text="item.metas.answer_count"></span>'.__('个网友回答','b2').'</div>
                                        <div>
		...
		if ( empty( $widget )) {
			...
			$widget .= '<div class="b2-widget-title  jitheme-widget-title">';
			$widget .= $atts['before_title']. esc_attr( $instance['ask_title'] ) .$atts['after_title'];
			$widget .= '</div><div class="comment-widget-nav"><span :class="{\'locked\':prev}" @click="prevAc()">'.b2_get_icon('b2-arrow-left-s-line').'</span><span :class="{\'locked\':next}" @click="nexAc()">'.b2_get_icon('b2-arrow-right-s-line').'</span></div>';
			$widget .= '<div class="b2-widget-box">'.$html.'</div>';
			$widget .='<div class="jitheme_widget_padding"><div class="jitheme-widget-btn b2-color"><a class="jitheme-jb-btn" href="/ask" target="_blank">全部问答</a></div></div>';
