...
class Comment
{
    ...
    public static function comment_callback($comment, $args, $depth)
    {
        ...
        ?>
        <li>
            <article class="<?php echo empty($args['has_children']) ? 'comment' : 'parent comment';
            echo $comment_sticky == $comment->comment_ID ? ' sticky-itme' : ''; ?>"
                id="comment-<?php echo $comment->comment_ID; ?>" itemscope itemtype="http://schema.org/Comment">
                ...
                <div class="comment-item b2-radius">
                    <div class="comment-meta" role="complementary">
                        ...
                        <div class="comment-floor">
                            <?php ; ?>
                            <?php echo b2_timeago($comment->comment_date); ?>
                        </div>
                    </div>
                    <div class="comment-content post-content" itemprop="text">
                        <div class="comment-content-text">
                            <?php do_action('b2_comment_text', $comment);
                            comment_text(); ?>
                        </div>
                        <div class="comment-footer">
                            <div class="comment-footer-tools">
                                <div class="comment-vote-hidden">
                                    <button id="comment-up-<?php echo $comment->comment_ID; ?>" class="'comment-up text"
                                        onclick="b2CommentList.vote(this,'comment_up','<?php echo $comment->comment_ID; ?>')">
                                        <?php echo b2_get_icon('b2-heart-fill') . '<span></span>'; ?>
                                    </button>
                                    <button id="comment-down-<?php echo $comment->comment_ID; ?>" class="'comment-down text"
                                        onclick="b2CommentList.vote(this,'comment_down','<?php echo $comment->comment_ID; ?>')">
                                        <?php echo b2_get_icon('b2-dislike-fill') . '<span></span>'; ?>
                                    </button>
                                </div>
                                <div class="comment-item-hidden">
                                    <button class="jubao-button text" style="padding:0"
                                        @click="jubao.close('comment',<?php echo $comment->comment_ID; ?>)"><?php echo __('举报', 'b2'); ?></button>
                                    <?php if (!$comment->comment_parent) { ?>
                                        <button class="text comment-zd"
                                            onclick="b2CommentList.sticky('<?php echo $comment->comment_ID; ?>')">
                                            <?php echo $comment_sticky == $comment->comment_ID ? __('取消置顶', 'b2') : __('置顶', 'b2'); ?>
                                        </button>
                                    <?php } ?>
                                    <button class="text fr reply" data-id="<?php echo $comment->comment_ID; ?>"
                                        onclick="b2CommentList.move('<?php echo $comment->comment_ID; ?>')">
                                        <?php echo __('回复', 'b2'); ?>
                                    </button>
                                </div>
                                <?php if ($comment->comment_approved == '0'): ?>
                                    <p class="comment-meta-item red">
                                        <?php echo __('审核中...', 'b2'); ?>
                                    </p>
                                <?php endif; ?>
                            </div>
    ...
    public static function get_commenter()
    {
        ...
        if (!empty($current_comment_user['comment_author_email'])) {
            $comment_user = array(
                'name' => wp_strip_all_tags($current_comment_user['comment_author']),
                'user_email' => wp_strip_all_tags($current_comment_user['comment_author_email']),
    ...
    public static function submit_comment($args)
    {
        ...
        if ($user_id) {
            ...
            if (!$role)
                return array('error' => __('您无权进行此操作', 'b2'));
        ...
        if (isset($img['imgUrl']) && !empty($img['imgUrl'])) {
            if (!attachment_url_to_postid($img['imgUrl']))
    ...
    public static function get_comments_json($post_id, $paged, $order_by = 'ASC')
    {
        ...
        if ($order_by !== 'ASC' && $order_by !== 'DESC')
            return array('error' => __('错误的参数', 'b2'));
    ...
    public static function get_hot_comment($post_id)
    {
        ...
        wp_cache_set($post_id, $data, 'b2_hot_comment', 12 * MINUTE_IN_SECONDS);
