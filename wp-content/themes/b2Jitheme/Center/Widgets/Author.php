...
class Jitheme_author extends WP_Widget{
    //小工具slug
	protected $widget_slug = 'ji-widget-author';

    //短代码名
	protected static $shortcode = 'ji_widget_author';
	...
	public static function get_widget( $atts ) {
			...
			<div class="author-widget">
				<div class="author-widget-content">
					...
					<div class="ji_author_btn">
            			<button @click="followingAc" class="button empty  red-color" v-if="following == 1" v-cloak>'.b2_get_icon(' b2-heart-fill').__('取消关注','b2').'</button>
            			<button @click="followingAc" v-else class="button red-color">'.b2_get_icon('b2-add-line').__('关注','b2').'</button>
            			<button class="button blue-color" @click="dmsg()"><i class="b2font b2-mail-send-line "></i>'.__('私信','b2').'</button>
            			<a href="'.$author_data['link'].'" class="button web-color"><i class="b2font b2-user-heart-line"></i>'.__('主页','b2').'</a>
        			</div>
		...
		if ( empty( $widget )) {
			...
			$widget .= '<div class="jitheme_widget_padding"><div class="b2-widget-title">';
			$widget .= $atts['before_title'].'<span></span>'.$atts['after_title'];
			$widget .= '</div>';
			$widget .= '<div class="b2-widget-box">'.$html.'</div></div>';
...
function Jitheme_widget_author() {
	register_widget( 'Jitheme_author' );
}
add_action( 'widgets_init', 'Jitheme_widget_author' );