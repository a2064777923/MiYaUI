...
class HotCircle extends \WP_Widget{
	...
	public static function get_widget( $atts ) {
		...
		if ( empty( $widget )) {
            ...
            $widget .= '<h2>
            <button :class="type === \'hot\' ? \'b2-radius picked \' : \'b2-radius \'" @click="getCirclesList(\'hot\')">'.sprintf(__('热门%s','b2'),$circle_name).'</button>
            <button :class="type === \'join\' ? \'b2-radius picked \' : \'b2-radius \'" @click="getCirclesList(\'join\')">'.__('我加入的','b2').'</button>
            <button :class="type === \'create\' ? \'b2-radius picked \' : \'b2-radius \'" @click="getCirclesList(\'create\')">'.__('我创建的','b2').'</button>
            </h2>';
			$widget .= '</div>';
			$widget .= '<div class="b2-widget-box b2-radius">'.$html.'</div>';
