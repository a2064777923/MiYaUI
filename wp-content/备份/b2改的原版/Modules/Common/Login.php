...
class Login{
    ...
    public static function check_login_name($current_user_id){
        ...
        //检查是否绑定了邮箱
        if($check_type === 'email'){
            if(empty($user_data->data->user_email)) return 'email';
            return false;
        }

        if($check_type === 'telandemail'){
            if((empty($user_data->data->user_email)) && !self::is_phone($user_data->data->user_login)) return 'telandemail';
            return false;
        }
    ...
    public static function check_email($email){
        ...
        if($white_list) return array('error'=>sprintf(__('不能使用%s的邮箱在本站注册','b2'),$_email));
