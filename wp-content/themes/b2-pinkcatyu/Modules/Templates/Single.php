...
class Single{
    ...
    public function init(){
        ...
        add_filter( 'the_content', array($this,'post_download'),9999,1); 
            ...
            <div>
                ...
                <div class="download-list" v-cloak>
                    <div v-for="(item,index) in list" :class="\'download-item b2-radius \'+(item.current_user.can.allow ? \'allow-down\' : \'not-allow-down\')" v-show="picked == index" v-cloak>
                        ...
                        <div class="download-rights-title">'.b2_get_icon('b2-download-cloud-line1').__('下载权限','b2').'</div><span class="mobile-show" @click="item.show_role = !item.show_role">'.__('查看','b2').'</span>
                        ...
                        <div class="download-info">
                        <div class="download-rights-title"><span v-text="item.name"></span><a :href="item.view" target="_blank" class="download-view button empty text" v-if="item.view">'.__('查看演示','b2').b2_get_icon('b2-arrow-right-s-line').'</a></div>
            ...
            <div class="content-footer post-content-footer" v-cloak>
                <div class="post-content-footer-in">
                    <div class="content-footer-poster">
                        ...
                        <button class="text favorite-button b2tooltipbox" data-title="'.__('举报','b2').'" @click="jubao.close(\''.$post_type.'\','.$post->ID.')" v-cloak>'.b2_get_icon('b2-shield-user-line').'<b class="mobile-show">'.__('举报','b2').'</b></button>
                    </div>
        ...
        ';
            ...
            <div class="content-footer">
                <div class="content-footer-poster">
                    ...
                    <button class="text favorite-button" @click="jubao.close(\'newsflashes\','.$post->ID.')" v-cloak>'.b2_get_icon('b2-shield-user-line').'<b>'.__('举报','b2').'</b></button>
                </div>
...
<div class="content-footer post-content-footer" v-cloak>
    <div class="post-content-footer-in">
        <div class="content-footer-poster">
            <button class="poster-span b2tooltipbox" @click="openPoster()" data-title="'.__('海报分享','b2').'">
                '.b2_get_icon('b2-share-line').'
                <b class="mobile-show">'.__('海报分享','b2').'</b>
            </button>
            
            <button @click="goComment()" class="mobile-hidden b2tooltipbox comment-span" data-title="'.__('去评论','b2').'">
                '.b2_get_icon('b2-chat-smile-2-line').'
            </button>
            
            <button :class="[\'text favorite-button\',{\'sc\':postData.favorites_isset},\'b2tooltipbox\']" @click="postFavoriteAc" data-title="'.__('收藏','b2').'">
                '.b2_get_icon('b2-star-line').'
                <b class="mobile-show">{{postData.favorites_isset ? \''.__('已收藏','b2').'\' : \''.__('收藏','b2').'\'}}</b>
            </button>
            <button class="text favorite-button b2tooltipbox" data-title="'.__('举报','b2').'" @click="jubao.close(\''.$post_type.'\','.$post->ID.')" v-cloak>
                '.b2_get_icon('b2-shield-user-line').'
                <b class="mobile-show">'.__('举报','b2').'</b>
            </button>
        </div>
        
        <div class="content-footer-zan-cai b2tooltipbox" data-title="'.__('喜欢','b2').'">
            <span @click="vote(\'up\')" :class="postData.up_isset ? \'picked\' : \'\'">
                '.b2_get_icon('b2-thumb-up-line').'<b v-text="postData.up"></b>
            </span>
            <span @click="vote(\'down\')" :class="postData.down_isset ? \'picked mobile-show\' : \'mobile-show\'">
                '.b2_get_icon('b2-thumb-down-line').'<b v-text="postData.down"></b>
            </span>
        </div>
    </div>
</div>
    ...
    public function shop_normal_action(){
        ...
        <div class="mg-t shop-single-action">
            ...
            <div class="shop-single-action-right b2flex" style="gap:20px;" v-if="postData !== ''">
                <button :class="['text favorite-button',{'sc':postData.favorites_isset}]" @click="postFavoriteAc" v-cloak style="padding:0"><?php echo b2_get_icon('b2-star-fill'); ?><span v-text="postData.favorites_isset ? '<?php echo __('已收藏','b2'); ?>' : '<?php echo __('收藏','b2'); ?>'"></span></button>    
                <!-- <button class="jubao-button text favorite-button" style="padding:0" @click="jubao.close('shop',<?php echo $post_id; ?>)"><?php echo b2_get_icon('b2-shield-user-line'); ?><?php echo __('举报','b2'); ?></button> -->
            </div>
        </div>
        ...
        <div class="shop-single-top" ref="shopSingle" data-id="<?php echo $post_id; ?>">
            ...
            <div class="shop-single-data">
                ...
                <div class="shop-title-box">
                    ...
                    <div v-if="data && data[id].limit.limit" class="red"><?php echo sprintf(__('限购商品：每个账户%s限购%s件','b2'),'<span v-if="data[id].limit.time">'.sprintf(__('%s天内','b2'),'<b v-text="data[id].limit.time"></b>').'</span>','<b v-text="data[id].limit.limit"></b>'); ?></div>
                </div>
                <?php 
                if($type){
