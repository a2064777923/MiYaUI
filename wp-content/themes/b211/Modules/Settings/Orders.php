...
class Orders{
    ...
    public function orders_statistics($cmb_options){
        ...
        $total = isset($total[0]) ? $total[0] : [
            'total'=>0,
            'refund'=>0
        ];

        $order_refund = $this->order_refund();
        $order_refund = isset($order_refund[0]) ? $order_refund[0] : [
            'day'=>0,
            'yesterday'=>0,
            'month'=>0,
            'last_month'=>0,
    ...
    public function get_order_guige($order_value,$post_id){
        ...
        if($type == 'normal' && $shop_multi == 1){
            $json = json_decode($order_value,true );
            if($json){
                $html = '';
                foreach ($json['desc'] as $key => $value) {
                    $html .= $value['name'].':'.$value['value'].PHP_EOL;
                }
                return $html;
            }
        }
    ...
    public function down_csv(){
        ...
        $head =  ['id','订单号','买家','买家ID','购买内容','商品ID','商品别名','规格','订单类型','商品类型','订单状态','订单日期','订单数量','单价','总价','货币类型','key','value','内容','支付渠道','快递单号','收货地址','标记','mobile'];
        $csv->outputData($head);
        ...
        foreach ($arg as $k => $v) {
            ...
            $data = [
                $v['id'],
                $v['order_id'],
                $this->dot2s((isset($author_obj->display_name) ? $author_obj->display_name : '')),
                $v['user_id'],
                $this->dot2s(get_the_title($v['post_id']).'(id:'.$v['post_id'].')'),
                $v['post_id'],
                urldecode(get_post_field('post_name',$v['post_id'])),
                $this->get_order_guige($v['order_value'],$v['post_id']),
...
* w : 等待付款 ，f : 已付款未发货 ，c : 已发货 ，s : 已删除 ，q : 已签收 ，t : 已退款
    ...
    public function list_option_page_cb($cmb_options){
        ...
        $status_total = isset($status_total[0]) ? $status_total[0] : [
            'total'=>0,
            'q'=>0,
            'w'=>0,
            'f'=>0,
            ...
            'verify'=>0,
            'circle_join'=>0,
            'circle_read_answer_pay'=>0,
            'circle_hidden_content_pay'=>0,
            'mission'=>0,
            'coupon'=>0,
            'custom'=>0,
            'infomation_sticky'=>0
                ...
                <?php if(isset($_REQUEST['action']) && $_REQUEST['action'] === 'edit'){ ?>
                    <?php 
                        ...
                        if($update){
                            ...
                            if (class_exists('B2APP\\common\\opt') && (int)$res['order_mark'] !== 1 && (($state == 'c' && (int)$res['order_commodity'] == 1) || (int)$res['order_commodity'] == 0 )){
                                ...
                                $res['receiver_contact'] = $phoneNumbers;
                                
                                $res_push = $m->pushExpress($res);
                            
                                if(!isset($res_push['error']) && $res_push){
                                    $wpdb->update(
                                        $table_name, 
                                        array(
                                            'order_mark'=>1,
                                    ...
                                    b2_settings_error('updated',__('微信小程序推送成功','b2'));
                                }
                                ...
                                <tbody>
                                    ...
                                    <?php 
                                            ...
                                            if($type == 'normal' && $shop_multi == 1){
                                                ...
                                                $json = json_decode($res['order_value'],true );
