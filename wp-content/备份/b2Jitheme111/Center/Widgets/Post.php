...
class Jitheme_post extends WP_Widget{

    //小工具slug
	protected $widget_slug = 'ji-widget-post';

    //短代码名
	protected static $shortcode = 'ji_widget_post';
	...
	public static function get_widget( $atts ) {
				...
				if($instance['show_thumb'] && $instance['post_type'] === 'post'){
					...
					if($instance['show_thumb'] == 2){
						...
						$html .= '<li class="b2-widget-box widget-post widget-post-none">
							<div class="b2-widget-post-order widget-order-'.$i.'"><span class="b2-radius">'.str_pad($i,2,"0",STR_PAD_LEFT).'</span></div>
							<div class="b2-widget-post-title">
								<h2>'.$title.'</h2>
								'.b2_timeago(get_the_date('Y-m-d G:i:s')).'
							</div>
		...
		if ( empty( $widget )) {
			...
			$widget .= !$instance['show_mobile'] ? str_replace('class="','class="mobile-hidden jitheme-widget-post ',$atts['before_widget']) : $atts['before_widget'];
...
function Jitheme_post_widget_boilerplate() {
	register_widget( 'Jitheme_post' );
}
add_action( 'widgets_init', 'Jitheme_post_widget_boilerplate' );