...
/*****************************************************************/
/* WP RECENT PAGES DASHBOARD WIDGET */
/*****************************************************************/

// WP RECENT PAGES WIDGET --> CALLABLE WIDGET CONTENT

if( ! function_exists('wp_admin_recent_pages_widget_content') ) :

    function wp_admin_recent_pages_widget_content() { 
                    ...
                    <?php while ( $my_posts_query->have_posts() ) : $my_posts_query->the_post(); 
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p><?php echo get_the_date(); ?> | <?php comments_number( esc_html__( 'No comments', 'wp-admin-theme-cd' ), esc_html__( 'One comment', 'wp-admin-theme-cd' ), esc_html__( '% comments', 'wp-admin-theme-cd' ) ); ?></p>
                            </td>
                        </tr>
...
// WP RECENT PAGES WIDGET --> ADD DASHBOARD WIDGET

if( ! function_exists('wp_admin_recent_pages_widget') ) :

	function wp_admin_recent_pages_widget() {

		wp_add_dashboard_widget('wp_recent_pages_db_widget', esc_html__( 'Recent Pages', 'wp-admin-theme-cd' ), 'wp_admin_recent_pages_widget_content');

	}

endif;

add_action('wp_dashboard_setup', 'wp_admin_recent_pages_widget');