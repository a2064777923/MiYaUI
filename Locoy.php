...
$post_status            = 'future';    //future-发布 pending-审核  draft-草稿箱
$time_interval          = 1;    
$post_next              = 'now';      
$post_ping              = false;    
...
$postformat             = false;    
$post                   = [];  // 初始化 $post 变量
$post_meta              = [];  // 初始化 $post_meta 变量
$post_cate_meta         = [];  // 初始化 $post_cate_meta 变量
$post_excerpt           = '';  // 初始化 $post_excerpt 变量
$tag                    = '';  // 初始化 $tag 变量
$fujianid               = '';  // 初始化 $fujianid 变量
...
// 在使用 extract() 前检查输入变量，避免未定义的变量覆盖现有变量
if(isset($_POST)) {
    $safe_vars = array_filter($_POST, function($key) {
        return !in_array($key, ['GLOBALS', '_SERVER', '_GET', '_POST', '_FILES', '_COOKIE', '_SESSION', '_ENV', '_REQUEST']);
...
if (!isset($post_taxonomy_list)) $post_taxonomy_list = [];

// 从 $_POST 获取数据
// 确保 $post 是一个数组
$post = isset($_POST['post']) && is_array($_POST['post']) ? $_POST['post'] : [];

// 检查自定义字段 - 确保 $post 是数组并且 'post_meta' 键存在
if(is_array($post) && isset($post['post_meta'])){
  $post_meta = $post['post_meta'];
}
// 检查自定义分类信息 - 确保 $post 是数组并且 'post_cate_meta' 键存在
if(is_array($post) && isset($post['post_cate_meta'])){
                  ...
                  if (function_exists('wp_generate_attachment_metadata')) {
                      try {
                          ...
                          if (is_array($attach_data)) {
                              ...
                              if (isset($attach_data['sizes']) && is_array($attach_data['sizes'])) {
...
function hm_add_category($post_category, $post_taxonomy = 'category')
{
    ...
    foreach ($post_category_list as $category) {
      ...
      if (!isInteger($cat_ID) || $cat_ID < 1) {
            ...
            $cat_ID = (is_array($term) && isset($term['term_id'])) ? $term['term_id'] : 0;
            if($cat_ID == 0){
                ...
                if(!empty($post_parent_cate) && $post_parent_cate != '[父分类]')
                {
                    ...
                    if($parent == 0){
                        $post_parent_cate = esc_sql($post_parent_cate);
...
function isInteger($value){
  return is_numeric($value) && (is_int($value) || ctype_digit(strval($value)));
}
...
{
  // PHP 8.0+ 不再支持 magic_quotes，直接返回字符串
  return $str;
}
...
function formatdate($date){
  ...
  if(strpos($date, 'yesterday') !== false){
    return str_replace('yesterday at', $dd, $date);
  }
  ...
  function strtoarray($str){
    if(strpos($str, '|||') !== false){
        ...
        if(strpos($str[0],'$$') !== false){
          ...
          foreach($str as $k => $v){
            $v = explode('$$', $v);
            if(isset($v[0]) && isset($v[1])) { // 确保数组元素存在
              $r[$v[0]] = $v[1];
            }
          }
          $str = $r;
...
function hm_do_save_post($post_detail)
{
    ...
    try {
        ...
        if (!isset($post_excerpt)) $post_excerpt = '';
