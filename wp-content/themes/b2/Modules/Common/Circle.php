...
class Circle
{
    ...
    public static function insert_topic_card($id)
    {
        ...
        if (!$post_type && strpos($url, '/' . $circle_link) === false)
            return array('error' => __('不支持此类连接', 'b2'));

        if ($id && $post_type && ($post_type == 'post' || $post_type == 'shop' || $post_type == 'document' || $post_type == 'newsflashes' || $post_type == 'announcement' || $post_type == 'page')) {
    ...
    public static function check_topic_role($user_id, $circle_id = 0, $circles = true)
    {
        ...
        if (isset($GLOBALS[$g_name]))
            return $GLOBALS[$g_name];
    ...
    public static function join_circle($circle_id)
    {
        ...
        if ($role['type'] === 'free') {
            ...
            if ($role['data'] === 'free') {
                ...
                if (
                    !$pending && CircleRelate::update_data(array(
                        'user_id' => $user_id,
                        'circle_id' => $circle_id,
                        'circle_role' => 'pending',
                        'join_date' => current_time('mysql')
            ...
            if (!$role['data']['allow_join'])
                return array('error' => __('您还未获得入圈资格', 'b2'));
    ...
    public static function insert_circle_topic($data)
    {
        ...
        if (!$data['circle'])
            return array('error' => sprintf(__('请选择话题所在的%s', 'b2'), $circle_name));
        ...
        if ($role['dark_room'])
            return array('error' => __('小黑屋禁闭中，无法操作', 'b2'));
        if (!$role['allow_create_topic'])
            return array('error' => __('权限不足', 'b2'));

        if (!isset($role['topic_type_role'][$topic_type]) || !$role['topic_type_role'][$topic_type])
            return array('error' => __('权限不足', 'b2'));

        if (!$role['in_circle'])
            return array('error' => sprintf(__('您没有权限在此%s发布话题', 'b2'), $circle_name));

        if (!$role['can_post'])
            return array('error' => __('您有待审的话题还未审核，请稍后再发布新话题', 'b2'));
        ...
        if (b2getStrLen($data['title']) > 200)
            return array('error' => __('标题太长，请限制在1-200个字符之内', 'b2'));
        ...
        if (b2getStrLen($content) < (int) $count[0] || b2getStrLen($content) > (int) $count[1])
            return array('error' => sprintf(__('话题字数请限制在%s到%s之间', 'b2'), $count[0], $count[1]));
        ...
        if (isset($check_read_role['error']))
            return $check_read_role;
        ...
        if ($data['topic_id']) {
            if (get_post_type($data['topic_id']) !== 'circle')
                return array('error' => __('文章类型错误', 'b2'));
    ...
    {
        if (!isset($data['role']['see']) || empty($data['role']['see']))
            return array('error' => __('请设置话题的阅读权限', 'b2'));
        ...
        switch ($data['role']['see']) {
            case 'money':
                if ((float) $data['role']['money'] <= 0 || (float) $data['role']['money'] > 99999)
                    return array('error' => __('阅读权限，设置的金额错误', 'b2'));
                break;
            case 'lv':
                if (empty($data['role']['lvPicked']))
                    return array('error' => __('请选择允许查看的用户组', 'b2'));
                ...
                foreach ($data['role']['lvPicked'] as $k => $v) {
                    if ($v !== 'verify') {
                        if (!isset($lvs[$v]))
                            return array('error' => sprintf(__('不存在%s用户组', 'b2'), $v));
                    }
                }
                break;
            case 'credit':
                if ((int) $data['role']['credit'] <= 0 || (int) $data['role']['credit'] > 99999)
                    return array('error' => __('阅读权限，设置的积分数额错误', 'b2'));
                break;
        }
    ...
    {
        if (!method_exists(__CLASS__, 'check_' . $data['type']))
            return array('error' => __('非法请求', 'b2'));
            ...
            if (in_array($k, $allow['media_role'])) {
                ...
                foreach ($files['list'] as $_k => $_v) {
                    if ($k === 'file') {
                        ...
                        if (isset($_v['data']['link']) && $_v['data']['link']) {
                            $list[$k][] = $_v['data']['link'];
                        }
                    } else {
    ...
    public static function check_ask($data)
    {
        ...
        if (!isset($ask['type']))
            return array('error' => __('提问类型错误', 'b2'));

        //检查提问用户
        if ($ask['type'] !== 'everyone' && $ask['type'] !== 'someone')
            return array('error' => __('提问类型错误', 'b2'));
        if ($ask['type'] === 'someone' && !isset($ask['pickedList']))
            return array('error' => __('请指定提问对象', 'b2'));
        ...
        if (isset($ask['pickedList'])) {
            foreach ((array) $ask['pickedList'] as $k => $v) {
                ...
                if ($user === false)
                    return array('error' => __('指定的用户不存在', 'b2'));
                $is_admin = user_can($current_user, 'manage_options');
                $is_circle_admin = self::is_circle_admin($current_user, $data['circle']);

                if ((int) $current_user === (int) $v['id'] && !$is_admin && !$is_circle_admin)
                    return array('error' => __('不能向自己提问', 'b2'));
                $ids[] = (int) $v['id'];
            }
        }

        if (isset($ask['pickedList']) && count((array) $ask['pickedList']) > 6)
            return array('error' => __('指定的用户过多', 'b2'));

        if ($ask['reward'] !== 'money' && $ask['reward'] !== 'credit')
            return array('error' => __('奖励类型错误', 'b2'));

        if (!is_numeric($ask['time']))
            return array('error' => __('请输入过期时间（纯数字）', 'b2'));
        if ($ask['time'] > 30 || $ask['time'] < 1)
            return array('error' => sprintf(__('过期天数应该大于%s并且小于%s', 'b2'), 1, 30));

        //如果是积分，检查积分
        if ($ask['reward'] === 'credit') {
            if ((int) $ask['pay'] <= 0 || (int) $ask['pay'] > 999999)
                return array('error' => __('积分数额错误', 'b2'));

            $credit = get_user_meta($current_user, 'zrz_credit_total', true);
            if ((int) $ask['pay'] > (int) $credit)
                return array('error' => __('积分不足', 'b2'));
        }

        if (!is_numeric($ask['pay']))
            return array('error' => __('积分或金额填写错误', 'b2'));

        if ($ask['reward'] === 'money') {
            if ((float) $ask['pay'] <= 0 || (float) $ask['pay'] > 999999)
                return array('error' => __('金额错误', 'b2'));

            $money = get_user_meta($current_user, 'zrz_rmb', true);
            if ((float) $ask['pay'] > (float) $money)
    ...
    public static function check_vote($data)
    {
        ...
        if (!isset($vote['type']) || ($vote['type'] !== 'pk' && $vote['type'] !== 'radio' && $vote['type'] !== 'multiple'))
        ...
        if (count($list) < 2)
            return array('error' => __('至少要设置两个投票选项', 'b2'));
    ...
    public static function check_guess($data)
    {
        ...
        $list = array();
        if (!is_numeric($guess['right']))
            return array('error' => __('请设置一个正确答案', 'b2'));
        ...
        if (count($list) < 2)
            return array('error' => __('请设置2个以上的题目', 'b2'));
    ...
    public static function create_circle($data)
    {
        ...
        if (!$role && !user_can($user_id, 'manage_options'))
            return array('error' => sprintf(__('您没有权限创建%s', 'b2'), $circle_name));
        ...
        if (!$data['tags'])
            return array('error' => sprintf(__('请选择%s类别', 'b2'), $circle_name));

        $tags = self::get_circle_tags();
        if (isset($tags['error']))
            return $tags;

        if (!in_array($data['tags'], $tags))
            return array('error' => __('不存在的类别', 'b2'));
        ...
        if ($data['pay']['type'] === 'money') {
            ...
            if ($empty)
                return array('error' => sprintf(__('请填写加入%s所需金额', 'b2'), $circle_name));
        }

        if ($data['pay']['type'] === 'lv') {
            if (empty($data['role']['lv']))
                return array('error' => sprintf(__('请选择允许加入%s的用户组', 'b2'), $circle_name));
        ...
        if ($data['read'] !== 'public' && $data['read'] !== 'private')
            return array('error' => sprintf(__('请设置%s隐私', 'b2'), $circle_name));
        ...
        if (!$data['info']['slug'])
            return array('error' => sprintf(__('请填写%s英文名称', 'b2'), $circle_name));

        $mb = mb_strlen($data['info']['slug'], 'utf-8');
        $st = strlen($data['info']['slug']);
        if ($st !== $mb)
            return array('error' => __('请使用纯英文', 'b2'));

        if (get_term_by('slug', $data['info']['slug'], 'circle_tags'))
            return array('error' => sprintf(__('%s英文名称有重复，请更换英文名称', 'b2'), $circle_name));

        if (get_term_by('name', $data['info']['name'], 'circle_tags'))
            return array('error' => sprintf(__('%s名称有重复，请更换名称', 'b2'), $circle_name));
    ...
    public static function check_best($data)
    {
        ...
        //如果已结算
        if (get_post_meta($data['topic_id'], 'b2_ask_settled', true))
            return;
    ...
    public static function get_data_by_topic_id($topic_id, $user_id = 0, $topic_query = null, $can_read = false)
    {
        ...
        if (get_post_type($topic_id) !== 'circle')
            return array('error' => __('话题不存在', 'b2'));
    ...
    public static function get_circles_list($data)
    {
        ...
        if ($data['count'] > 50)
            return array('error' => __('每页显示数量过多', 'b2'));
    ...
    public static function topic_vote($topic_id, $index)
    {
        ...
        if ($check)
            return array('error' => __('您已投过票了', 'b2'));

        //检查话题类型
        $topic_type = get_post_meta($topic_id, 'b2_circle_topic_type', true);
        if ($topic_type !== 'vote')
            return array('error' => __('当前话题不是投票', 'b2'));

        $list = get_post_meta($topic_id, 'b2_circle_vote_list', true);

        if (empty($list))
            return array('error' => __('数据不完整', 'b2'));
    ...
    public static function topic_guess($topic_id, $index)
    {
        ...
        if ($check)
            return array('error' => __('您已猜过一次了', 'b2'));

        //检查话题类型
        $topic_type = get_post_meta($topic_id, 'b2_circle_topic_type', true);
        if ($topic_type !== 'guess')
            return array('error' => __('当前话题不是<你猜>', 'b2'));
    ...
    public static function answer_count($topic_id)
    {
        ...
        wp_cache_set($topic_id, $count, 'b2_answer_count', 9 * MINUTE_IN_SECONDS);
    ...
    public static function submit_topic_answer($data)
    {
        ...
        if ((isset($data['image']) && $data['image'] && !is_numeric($data['image'])) || (isset($data['file']) && $data['file'] && !is_numeric($data['file'])))
            return array('error' => __('图片数据错误', 'b2'));
        ...
        if ($content == '')
            return array('error' => __('回答不能为空', 'b2'));
        ...
        if ($type !== 'ask')
            return array('error' => __('该话题不是问答', 'b2'));
    ...
    public static function can_read_answer($topic_id, $user_id)
    {
        ...
        //如果是向所有人提问
        $ask_type = get_post_meta($topic_id, 'b2_circle_ask_type', true);
        if ($ask_type === 'everyone')
            return true;

        //检查是不是提问者本人
        $author = get_post_field('post_author', $topic_id);
        if ((int) $author === (int) $user_id)
            return true;
        ...
        if ($filter_data['is_admin'] || $filter_data['is_circle_admin'])
            return true;
    ...
    public static function get_topic_answer_list($topic_id, $paged)
    {
        ...
        if (!self::can_read_answer($topic_id, $user_id))
            return false;
    ...
    public static function answer_right($answer_id)
    {
        ...
        if (self::ask_time_pass($parent) === -1)
            return array('error' => __('问题已过期，无法采纳', 'b2'));
        ...
        if ($best)
            return array('error' => __('该问题已有最佳答案', 'b2'));
    ...
    public static function circle_search($key)
    {
        $args = array(
            ...
            'fields' => 'all',
            'count' => 10,
            'name__like' => b2_remove_kh(esc_sql($key))
        );
