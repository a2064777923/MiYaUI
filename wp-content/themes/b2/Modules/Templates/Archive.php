...
class Archive
{
    ...
    public function archive_filters()
    {
        ...
        if (!isset($filters[0]['show']) || !$filters[0]['show'])
            return '';
    ...
    public static function filter_meta($meta, $request, $url)
    {
        ...
        $_meta = isset($_GET['meta_key']) ? wp_strip_all_tags($_GET['meta_key']) : '';

        $html = '';
        foreach ($meta_filters as $k => $v) {

            $a = '<div class="filter-items"><a href="' . (remove_query_arg($v['meta_key'], $request) ?: $url) . '" data-key="all" class="' . (!$_meta ? 'current' : '') . '">' . __('全部', 'b2') . '</a>';
            foreach ($v['meta_value'] as $_k => $_v) {
                $a .= '<a class="' . ($_meta && $_meta == $_v['meta_key'] ? 'current' : '') . '" href="' . (add_query_arg($v['meta_key'], $_v['meta_key'], $url . $request)) . '" data-key="' . $_v['meta_key'] . '" title="' . $_v['meta_name'] . '">' . $_v['meta_name'] . '</a>';
            }
            $a .= '</div>';
    ...
    public static function filter_orderby($request, $url, $term, $filters)
    {
        ...
        $order = isset($_GET['post_order']) ? wp_strip_all_tags($_GET['post_order']) : '';

        if (in_array('date', $filters[0]['order'])) {
            $html .= '<a href="' . (add_query_arg('post_order', 'new', $url . $request)) . '" class="' . (!$order || ($order && $order == 'new') ? 'current' : '') . '">' . __('最新', 'b2') . '</a>';
        }

        if (in_array('random', $filters[0]['order'])) {
            $html .= '<a href="' . (add_query_arg('post_order', 'random', $url . $request)) . '" class="' . ($order && $order == 'random' ? 'current' : '') . '">' . __('随机', 'b2') . '</a>';
        }

        if (in_array('views', $filters[0]['order'])) {
            $html .= '<a href="' . (add_query_arg('post_order', 'views', $url . $request)) . '" class="' . ($order && $order == 'views' ? 'current' : '') . '">' . __('最多浏览', 'b2') . '</a>';
        }

        if (in_array('like', $filters[0]['order'])) {
            $html .= '<a href="' . (add_query_arg('post_order', 'like', $url . $request)) . '" class="' . ($order && $order == 'like' ? 'current' : '') . '">' . __('最多喜欢', 'b2') . '</a>';
        }

        if (in_array('comments', $filters[0]['order'])) {
            $html .= '<a href="' . (add_query_arg('post_order', 'comments', $url . $request)) . '" class="' . ($order && $order == 'comments' ? 'current' : '') . '">' . __('最多评论', 'b2') . '</a>';
        }
    ...
    public function archive_top_info()
    {
        ...
        $search = b2_key_sanitize(get_search_query());
                    ...
                    <div class="tax-info">
                        ...
                        <?php if ($fliter) { ?>
                            <div class="tax-info-item mobile-show" @click.stop="show('cat')">
                                <button
                                    class="fliter-button"><?php echo __('筛选', 'b2') . b2_get_icon('b2-arrow-down-s-line'); ?></button>
                            </div>
                        <?php } ?>
                        <div class="tax-search tax-info-item">
                            <form method="get" action="<?php echo $url . $request; ?>" autocomplete="off">
                                <input type="text" name="archiveSearch" class="b2-radius"
                                    placeholder="<?php echo sprintf(__('在「%s」中搜索', 'b2'), $title); ?>"
                        ...
                        <div class="tax-title">
                            <div class="tax-info-item">
                                <a class="fliter-button button" href="<?php echo b2_get_custom_page_url('tags'); ?>"
                                    target="_blank"><?php echo __('全部标签', 'b2'); ?></a>
                            </div>
                            <h1 class="b2-radius" style="background-image:url(<?php echo $img; ?>)">
                                <span><?php echo $title; ?></span></h1>
                        </div>
                    </div>
                    <?php if (isset($filters[0]['order']) && $filters[0]['order']) { ?>
