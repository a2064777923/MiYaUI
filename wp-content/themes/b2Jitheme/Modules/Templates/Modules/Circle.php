...
class Circle{
    ...
    public function init($data,$i,$return = false){
        ...
        $img=!empty($data['module_title_img']) ? $data['module_title_img'] : $default_image;
        ...
        $show_title = $show_title ? '<div class="Onecad_title">
        ...
        $html .= '<div class="shop-box-title jitheme-post-title">
            <div class="modules-title-box">
                '.jithem_home_title($data['title'],$data['desc'],$img,$default_image,'/circle').'
            </div>
        </div>
        ...
        return '
            <div class="item-top b2-radius">
                    ...
                    <div class="author-info">
                        <p><i class="b2font b2-file-list-2-line"></i>'.$data['desc'].'</p>
                    </div>
                </div>
        ';
    }
    ...
    public function topic_html($data){
        ...
        foreach ($data as $k => $v) {
            ...
            $li .= '<div><span class="c-'.$type['type'].'">'.$type['text'].'</span>
                ...
                <span class="sort-config-desc">'.$type['time'].'</span>
                </div>
            ';
        }unset($v);
    ...
    public function get_topic_type($topic_id){
        ...
        // 使用 $topic_id 获取文章的作者 ID
        $author_id = get_post_field('post_author', $topic_id);
        
        // 获取作者的完整信息
        $author_name = get_the_author_meta('display_name', $author_id);
        $post_date = get_the_date('m-d', $topic_id);
        $post_time = get_post_time('m-d', false, $topic_id);
        switch ($type) {
            case 'ask':
                return array(
                    'type'=>'ask',
