...
class Sms
{
    ...
    public static function others($phone, $code)
    {
        ...
        //如果返回的内容和设置完全一致，发送成功
        if ($resout['body'] === b2_get_option('normal_login', 'others_back'))
            return true;

        if (strpos($resout['body'], b2_get_option('normal_login', 'others_back')) !== false)
            return true;

        //短信宝专用
        if (isset($resout['response']['message']) && $resout['response']['message'] == b2_get_option('normal_login', 'others_back'))
            return true;
