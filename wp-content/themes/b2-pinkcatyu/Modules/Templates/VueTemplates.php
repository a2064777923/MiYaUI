...
class VueTemplates
{
    ...
    public static function scripts_init()
    {
        ...
        $report_types = b2_get_option('report_main', 'report_type');

        //去掉前后的空格和空白换行
        $report_types = trim($report_types);

        $report_types = explode(PHP_EOL, $report_types);
        ...
        wp_localize_script('vue', 'b2_global', apply_filters('b2_global_settings', array(
            ...
            'tencent_captcha_appid' => b2_get_option('normal_login', 'CaptchaAppId'),
            'search_type' => b2_get_search_type(),
            'login_keep_days' => (int) b2_get_option('normal_login', 'login_keep'),
            'login_type'=>(int)b2_get_option('normal_login', 'allow_social_login'),
            'language' => get_locale(),
            'post_id' => $post_id,
            ...
            'js_text' => $text,
            'newsflashes_can_post' => (int) b2_get_option('newsflashes_main', 'newsflashes_can_post_open'),
            'links_can_post' => (int) b2_get_option('links_submit', 'link_submit_open'),
            'wx_mp_in_login' => b2_get_option('normal_login', 'wx_mp_in_login'),
            'mp_qrcode' => b2_get_option('normal_login', 'wx_show_qrcode'),
            'wx_appid' => b2_get_option('normal_login', 'wx_pc_key'),
            'post_gg' => $single ? self::post_gg() : '',
            'jubao_box' => self::jubao_box(),
            ...
            'cz' => [
                'max' => b2_get_option('normal_gold', 'gold_cz_max') ? b2_get_option('normal_gold', 'gold_cz_max') : 100,
                'count' => b2_get_option('normal_gold', 'gold_cz_count') ? b2_get_option('normal_gold', 'gold_cz_count') : 2
            ]
        )));
    }
                    ...
                    <form @submit.stop.prevent="loginSubmit">
                        ...
                        <div class="login-box-in" v-show="!(invitation != 0 && (loginType == 2 && !invitationPass)) && loginSocialType == 0">
                            ...
                            <label class="login-form-item" v-show="loginType == 2">
                                ...
                                <span><b><i class="b2font b2-user-heart-line " style="margin-right: 4px;"></i>' . __('用户昵称', 'b2') . '</b></span>
                            </label>

                            <label class="login-form-item" v-show="loginType != 4">
                                <input type="text" name="username" tabindex="2" v-model="data.username" :class="data.username ? \'active\' : \'\'" spellcheck="false" autocomplete="off">
                                <span v-if="loginType == 3"><b><i class="b2font b2-mail-send-line " style="margin-right: 4px;"></i>' . __('请输入邮箱', 'b2') . '</b></span>
                                <span v-else><b><i class="b2font b2-skull-2-line " style="margin-right: 4px;"></i>' . __('请输入登录', 'b2') . '{{loginText}}</b></span>
                            ...
                            <label :class="[\'login-form-item login-check-input\',{\'show\':(((loginType == 2 || loginType == 3) && data.username && checkType != \'luo\') || (checkType == \'luo\' && loginType == 3))}]">
                                ...
                                <span><b><i class="b2font b2-questionnaire-line " style="margin-right: 4px;"></i>' . __('验证码', 'b2') . '</b></span>
                                <div class="check-code-img" v-if="checkType == \'text\' && loginType != 3" @click="changeCode">
                                    <img :src="codeImg" v-if="codeImg"/>
                                    <i class="recaptcha-load" v-else></i>
                                </div>
               
                                <b id="CaptchaId" class="login-eye button text" @click.stop.prevent="!SMSLocked && count == 60 && checkType != \'tencent\' ? sendCode() : \'\'" v-else>{{count < 60 ? count+\'' . __('秒后可重发', 'b2') . '\' : \'' . __('发送验证码', 'b2') . '\'}}</b>
                            </label>

                            <label class="login-form-item" v-show="loginType != 3">
                                <input name="password" :type="eye ? \'text\' : \'password\'" tabindex="4" v-model="data.password" :class="data.password ? \'active\' : \'\'" autocomplete="off" spellcheck="false">
                                <span><b v-if="loginType == 4">' . __('新密码', 'b2') . '</b><b v-else><i class="b2font b2-lock-2-line " style="margin-right: 4px;"></i>' . __('请输入密码', 'b2') . '</b></span>
                            ...
                            <input type="checkbox" name="xieyi" v-model="$store.state.xieyi"/>
                            <span style="margin-left: 5px;">' . sprintf(__('点击同意 %s用户协议%s 和 %s隐私政策%s', 'b2'), '<a href="' . $site_terms . '" target="_blank">', '</a>', '<a href="' . $site_privacy . '" target="_blank">', '</a>') . '</span>
                            </label>
                            </div>
                            ...
                            <div :class="loginType == 3 || loginType == 4 || (invitationPass && loginType == 2) ? \'login-tk-forget login-tk\' : \'login-tk\'">
                                ...
                                <p class="login-p" v-if="(loginType == 1 || loginType == 3) && allowRegister == 1"><a v-if="loginType == 1" href="javascript:void(0)" @click="loginAc(3)">' . __('忘记密码？', 'b2') . '</a><span>' . __('还没有账号？', 'b2') . '<a href="javascript:void(0)" @click="loginAc(2)">' . __('去注册', 'b2') . '</a></span></p>
                                <p v-if="loginType == 2 || loginType == 3 || loginType == 4"><a v-if="loginType == 2" href="javascript:void(0)" @click="loginAc(3)">' . __('忘记密码？', 'b2') . '</a><span>' . __('已有帐号？', 'b2') . '<a href="javascript:void(0)" @click="loginAc(1)">' . __('返回登录', 'b2') . '</a></span></p>
                            </div>
                        </div>
                        <div class="SignContainer-tip">已阅读并同意<a href="'.$site_terms.'">《网站协议》</a><a href="'.$site_privacy.'">《隐私保护指引》</a></div>
                        <div :class="\'login-social-button \'+(openOauth ? \'show\' : \'\')" v-if="!(invitation != 0 && (loginType == 2 && !invitationPass)) && openOauth && loginSocialType == 0">
                            <div :class="[\'login-social-button-bottom\',{\'is-weixin\':isWeixin}]">
                                <div>' . __('其他登录方式', 'b2') . '</div>
                                <div>
                        ...
                        <div :class="\'login-social-button \'+(openOauth ? \'show\' : \'\')" v-if="!(invitation != 0 && (loginType == 2 && !invitationPass)) && openOauth">
                            <div :class="[\'login-social-button-bottom\',{\'is-weixin\':isWeixin}]">
                                ...
                                <div class="site-terms" v-if="loginType == 2 && allowRegister == 1">
                                <label><input type="checkbox" name="xieyi" v-model="$store.state.xieyi"/>' . sprintf(__('点击同意 %s用户协议%s 和 %s隐私政策%s', 'b2'), '<a href="' . $site_terms . '" target="_blank">', '</a>', '<a href="' . $site_privacy . '" target="_blank">', '</a>') . '</label>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    public static function jubao_box()
    {
        return '<div :class="[\'modal\',\'jubao-form\',{\'show-modal\':show}]">
            <div class="modal-content b2-radius">
                <span class="close-button" @click="close(0)">×</span>
               <h2>举报</h2>
               <div>
               ...
               <p class="jubao-text" style="margin-top:10px;font-size:12px;color:#999">' . __('举报提交后，我们会以邮件的形式向您反馈处理结果。', 'b2') . '</p>
            </div>
        </div>';
    }
    ...
    public static function search_box()
    {
        ...
        return apply_filters('b2_search_box', $search_box);
    }
                    ...
                    <div :class="\'pay-box-content\'+\' \'+showtype">
                        ...
                        <template v-else-if="showtype == \'cz\'">
                            ...
                            <div style="margin-top:20px;font-size:12px">
                                ' . sprintf(__('您每天可以充值%s次，每次最多充%s', 'b2'), '<b v-text="b2_global.cz.count"></b>', B2_MONEY_SYMBOL . '<b v-text="b2_global.cz.max"></b>') . '
                            </div>
                        </template>
                        ...
                        <div class="pay-button">
                            <div>
                                <a v-if="jump == \'jump\' || jump == \'mweb\' || jump == \'jsapi\'" :href="!disabled() && href ? href : \'javascript:void(0)\'" :class="\'button \'+(disabled() ? \'disabled\' : \'\')" :target="!disabled() && href ? \'_blank\' : \'\'" @click="pay()"><span v-if="!payMoney && (showtype == \'ds\' || showtype == \'cz\' || showtype == \'cg\')">' . __('请输入金额', 'b2') . '</span><span v-else-if="!payType">' . __('请选择支付方式', 'b2') . '</span><span v-else-if="waitOrder">' . __('创建订单中...', 'b2') . '</span><span v-else-if="payType">' . __('支付', 'b2') . '</span></a>
                                <button :class="locked ? \'b2-loading\' : \'\'" :disabled="disabled() || locked ? true : false" @click="pay()" v-else><span v-if="!payMoney && (showtype == \'ds\' || showtype == \'cz\' || showtype == \'cg\')">' . __('请输入金额', 'b2') . '</span><span v-else-if="!payType">' . __('请选择支付方式', 'b2') . '</span><span v-else-if="payType">' . __('支付', 'b2') . '</span></button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    public static function mp_box()
    {
        ...
        $tip = __('打开微信扫一扫', 'b2');
        $mobile_tip = __('截图保存到手机相册，再微信扫码登录', 'b2');
        return '
                ...
                <div class="modal-content b2-radius">
                    ...
                    <div class="mp-box-content">
                        ...
                        <div v-show="!token">
                            ...
                            ' . ($is_mobile && !$is_weixin ? '<p>' . $mobile_tip . '</p>' : '<p>' . $tip . '</p>') . '
