...
class Document
{
    ...
    public static function submit_request($data)
    {
        ...
        if (!$content)
            return array('error' => __('请填写工单内容', 'b2'));

        $data['title'] = str_replace(array('{{', '}}'), '', $data['title']);

        $title = apply_filters('b2_sanitize_data', $data['title']);

        if (!$title)
            return array('error' => __('请填写标题', 'b2'));
        ...
        $image = '';
        if ((int) $data['image']) {
            $img_data = wp_get_attachment_url((int)$data['image']);
            if ($img_data) {
