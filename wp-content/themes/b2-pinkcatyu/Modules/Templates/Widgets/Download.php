...
<div class="w-d-download">
    <span v-for="b in item.button">
        <button @click="go(b.link,item.current_user.can.allow,item,index)" class="button" style="display: inline-flex; align-items: center; justify-content: center; gap: 6px;">
            <i class="b2font b2-download-cloud-line"></i>
            <span v-text="b.name"></span>
        </button>
    </span>
</div>
                    <p style="font-size: 13px; color: rgb(133, 144, 166); margin-top: 12px; text-align: center;">📢 下载次数不够？ <a href="/vips" target="_blank" style="text-decoration: underline;">点此开通会员</a></p>
                </div>
            </div>
		';
