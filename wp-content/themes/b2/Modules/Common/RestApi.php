...
class RestApi
{
    ...
    public function b2_rest_regeister()
    {
        ...
        //获取单个订单信息
        register_rest_route('b2/v1', '/getOrderInfo', array(
            'methods' => 'post',
            'callback' => array(__CLASS__, 'getOrderInfo'),
            'permission_callback' => '__return_true'
        ));
        ...
        //举报
        register_rest_route('b2/v1', '/submitReport', array(
            'methods' => 'post',
            'callback' => array(__CLASS__, 'submitReport'),
            'permission_callback' => '__return_true'
        ));
    }
    ...
    {
        $res = Report::submit_report($request->get_params());

        if (isset($res['error'])) {
            return new \WP_Error('report_error', $res['error'], array('status' => 403));
        } else {
    ...
    {
        if (!b2_check_repo())
            return new \WP_Error('user_error', __('操作频次过高', 'b2'), array('status' => 403));
    ...
    public static function getPostList($request)
    {
        ...
        if (!method_exists('B2\Modules\Templates\Modules\Posts', $type))
            return;
    ...
    public static function getVideoHtml($request)
    {
        ...
        if (strpos('[', $request['url']) !== false || strpos(']', $request['url']) !== false)
            return;
    ...
    public static function commentVote($request)
    {
        ...
        if (!b2_check_repo())
            return new \WP_Error('invitation_error', __('点的太快啦！', 'b2'), array('status' => 403));
    ...
    {
        $res = Recaptcha::code_letter(4, 186, 50);
        if (isset($res['error'])) {
            return new \WP_Error('regeister_error', $res['error'], array('status' => 403));
        } else {
            return new \WP_REST_Response($res, 200);
    ...
    {
        // if (!b2_check_repo($request['token']))
        //     return new \WP_Error('user_error', __('操作频次过高', 'b2'), array('status' => 403));
    ...
    {
        if (!b2_check_repo($request['order_id']))
            return new \WP_Error('user_error', __('操作频次过高', 'b2'), array('status' => 403));
    ...
    public static function BatchPayment($request)
    {
        ...
        if (!b2_check_repo($user_id . 'batch_payment'))
            return new \WP_Error('user_error', __('操作频次过高', 'b2'), array('status' => 403));
    ...
    //获取单个订单信息
    public static function getOrderInfo($request)
    {
        $res = Orders::get_order_info($request['order_id']);
    ...
    public static function ajaxupdate($request)
    {
        ...
        if (!method_exists('B2\Modules\Common\UpdateData', $type))
