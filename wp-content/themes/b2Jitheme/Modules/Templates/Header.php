...
class Header{
    ...
    public static function sub_menu($_arg = ''){
        ...
        if(b2_is_page_type('newsflashes')){
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <button class="empty jitheme-btn" onclick="topMenuLeft.go(\'requests\')">'.b2_get_icon('b2-clipboard-line').__('提交工单','b2').'</button>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <button class="empty jitheme-btn" onclick="topMenuLeft.go(\'orders\')">'.b2_get_icon('b2-file-list-2-line').__('我的订单','b2').'</button>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode){
                ...
                $html .= '<div class="top-submit">
                    <a class="empty jitheme-btn" href="'.b2_get_custom_page_url('all-circles').'" target="_blank">'.b2_get_icon('b2-donut-chart-fill').sprintf(__('所有%s','b2'),$circle_name).'</a>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <a class="empty jitheme-btn" href="'.b2_get_custom_page_url('po-ask').'" target="_blank">'.b2_get_icon('b2-user-location-line').__('提问','b2').'</a>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode && is_singular('infomation')){
                $html .= '<div class="top-submit">
                    <a class="empty jitheme-btn" href="'.b2_get_custom_page_url('po-infomation').'" target="_blank">'.b2_get_icon('b2-user-location-line').__('发布','b2').'</a>
                </div>';
            }
        }elseif(apply_filters('b2_is_page', 'post') || b2_is_page_type('announcement')){
            $arg = array(
                'theme_location' => 'post',
                'container_id'=>'top-menu',
                'container_class'=> 'top-menu',
            ...
            if($html && !$_arg && !$audit_mode && (int)b2_get_option('normal_write','write_allow') == 1){
                $html .= '<div class="top-submit">
                    <a class="empty jitheme-btn" href="javascript:void(0)" onclick="postPoBox.go(\''.b2_get_custom_page_url('write').'\',\'write\')">'.b2_get_icon('b2-quill-pen-line').__('投稿','b2').'</a>
                </div>';
            }
            ...
            if($html && !$_arg && !$audit_mode){
                $html .= '<div class="top-submit">
                    <a class="empty jitheme-btn" href="javascript:void(0)" onclick="userTools.goUserPage()">'.b2_get_icon('b2-user-heart-line').__('个人中心','b2').'</a>
                </div>';
            }
        }
