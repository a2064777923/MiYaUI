...
define('B2_VERSION', '5.9.42');
if(!defined('B2_THEME_DEBUG')){
	define('B2_THEME_DEBUG',false);
}
if (!defined('B2_DEFAULT_IMG')) {
	define('B2_DEFAULT_IMG', B2_THEME_URI . '/Assets/fontend/images/default-img.jpg');
}
if (!defined('B2_DEFAULT_AVATAR')) {
	define('B2_DEFAULT_AVATAR', B2_THEME_URI . '/Assets/fontend/images/default-avatar.png');
}
if (!defined('B2_GAP')) {
	define('B2_GAP', 16);
}
if (!defined('B2_SIDEBAR_GAP')) {
	define('B2_SIDEBAR_GAP', 16);
}
if (!defined('B2_IMG_RATIO')) {
	define('B2_IMG_RATIO', 1.2);
}
if (!defined('B2_VERIFY_CODE')) {
	define('B2_VERIFY_CODE', 'normal');
}
if (!defined('B2_ASIDE_SEARCH')) {
	define('B2_ASIDE_SEARCH', 1);
}
if (!defined('B2_EMPTY')) {
	define('B2_EMPTY', '<div class="empty-page"><img src="' . B2_THEME_URI . '/Assets/fontend/images/b2-page-empty.svg"><p>Empty Result</p></div>');
}
if (!defined('B2_VERIFY_ICON')) {
	define('B2_VERIFY_ICON', '<i class="b2-vrenzhengguanli b2font b2-color"></i>');
}
if (!defined('B2_LOADING_IMG')) {
...
function b2_get_option($where, $key, $circle_id = 0)
{
	...
	if (isset($settings[$key])) {
		...
		if (!isset(B2_DEFAULT_OPTS[$where][$key]))
			return '';
...
function b2_setup()
{
	load_theme_textdomain('b2', get_template_directory() . '/languages');
	...
	// $path = str_replace(['/', '\\'], DIRECTORY_SEPARATOR, B2_THEME_DIR .B2_DS. 'languages');

	// var_dump($path);

	// $file = apply_filters( 'theme_locale', get_locale(), 'b2' );
	// var_dump($file);
	 ...
	 * 初始化类
	 */
	require_once B2_THEME_DIR . B2_DS . 'loader.php';
...
function b2_widgets_init()
{
	...
	if (b2_get_option('activity_main', 'activity_open')) {
...
function b2_theme_init()
{
	...
	// load_theme_textdomain('b2', WP_LANG_DIR . '/themes');
	...
	// $activity_slug = b2_get_option('normal_custom', 'custom_activity_link');

	// $arr = array(
	// 	'name' => sprintf(__('%s分类', 'b2'), $activity_name),
	// 	'singular_name' => sprintf(__('%s分类', 'b2'), $activity_name),
	// 	'search_items' => sprintf(__('搜索%s分类', 'b2'), $activity_name),
	// 	'all_items' => sprintf(__('所有%s分类', 'b2'), $activity_name),
	// 	'parent_item' => sprintf(__('父级%s分类', 'b2'), $activity_name),
	// 	'parent_item_colon' => sprintf(__('父级%s分类', 'b2'), $activity_name),
	// 	'edit_item' => sprintf(__('编辑%s分类', 'b2'), $activity_name),
	// 	'update_item' => sprintf(__('更新%s分类', 'b2'), $activity_name),
	// 	'add_new_item' => sprintf(__('添加%s分类', 'b2'), $activity_name),
	// 	'new_item_name' => sprintf(__('%s分类名称', 'b2'), $activity_name),
	// 	'menu_name' => sprintf(__('%s分类', 'b2'), $activity_name),
	// );
	...
	// register_taxonomy('activity_cat', array('activity_cat'), $args);

	// $activity = array(
	// 	'name' => sprintf(__('%s中心', 'b2'), $activity_name),
	// 	'singular_name' => $activity_name,
	// 	'add_new' => sprintf(__('添加一个%s', 'b2'), $activity_name),
	// 	'add_new_item' => sprintf(__('添加一个%s', 'b2'), $activity_name),
	// 	'edit_item' => sprintf(__('编辑%s', 'b2'), $activity_name),
	// 	'new_item' => sprintf(__('新的%s', 'b2'), $activity_name),
	// 	'all_items' => sprintf(__('所有%s', 'b2'), $activity_name),
	// 	'view_item' => sprintf(__('查看%s', 'b2'), $activity_name),
	// 	'search_items' => sprintf(__('搜索%s', 'b2'), $activity_name),
	// 	'not_found' => sprintf(__('没有%s', 'b2'), $activity_name),
	// 	'not_found_in_trash' => __('回收站为空', 'b2'),
	// 	'menu_name' => $activity_name,
	// );
...
function b2_order_type($id = 0)
{
	...
	return apply_filters('b2_order_type', array(
		...
		'mission' => __('签到填坑', 'b2'),
		'coupon' => __('优惠劵使用', 'b2'),
		'custom' => $id ? get_the_title($id) : __('自定义支付', 'b2'),
...
function b2_text($data = array())
{
	...
	return array(
		...
		'global' => array(
			...
			'download_right_error' => __('请添加下载权限', 'b2'),
			...
			'jubao_success' => __('举报成功，我们会通过邮件给向反馈举报结果！', 'b2')
		),
		'b2_text' => array(
			...
			'user_dark_room' => __('请求本站接口过于频繁，关入小黑屋！', 'b2'),
			'user_dark_room_title' => __('关入小黑屋', 'b2'),
			'ask_pay' => __('您发起了一个付费提问', 'b2'),
			'ask_pay_title' => __('付费提问', 'b2'),
			'link_register' => __('申请入驻', 'b2'),
			'cz_count' => __('超过每日充值次数限制，请明天再来充值', 'b2'),
			'cz_max' => __('超过每日充值金额上限，请减少充值金额', 'b2'),
			'ask_inv' => __('${from}邀请您回答问题：${post_id}', 'b2'),
			'ask_inv_title' => __('邀请回答', 'b2'),
...
function b2_weixin_message_templates()
{
	return apply_filters('b2_weixin_message_templates', [
		'pay_success' => [
			...
			'keyword_name_list' => ['商品名称', '订单号', '订单金额', '下单时间']
		],
		'new_order' => [
			'name' => '客户新订单处理通知',
			'id' => '',
			...
			'keyword_name_list' => ['产品名称', '订单编号', '订单金额', '下单时间']
		],
		'order_ship' => [
			'name' => '订单发货通知',
			'id' => '',
...
function b2_settings_error($type = 'updated', $message = '')
{
	...
	if (empty($message))
		$message = $type == 'updated' ? __('设置已保存。', 'b2') : __('保存失败，请重试。', 'b2');
	add_settings_error(
		'b2_settings_message',
...
function b2_check_repo($key = 0)
{
	...
	$res = wp_cache_get('b2_rp_' . $key, 'b2_rp');

	if ($res)
		return false;
	wp_cache_set('b2_rp_' . $key, 1, 'b2_rp', 2);
...
{
	if (defined('OSS_ACCESS_ID') || defined('WPOSS_ACCESS_ID'))
		return $link . '?x-oss-process=video/snapshot,t_1,m_fast';
...
function b2_get_excerpt($post_id, $length = 200)
{
	...
	if (isset($GLOBALS['b2_get_excerpt_' . $post_id . '_' . $length]))
		return $GLOBALS['b2_get_excerpt_' . $post_id . '_' . $length];
...
function b2_search_page_title($title)
{
	if (is_search()) {
		// 设置自定义的搜索页面标题
		$title = __('搜索', 'b2');
...
add_filter('wp_title', 'b2_search_page_title', 10, 2);
add_filter('pre_get_document_title', 'b2_search_page_title');
...
function b2_recursive_sanitize_text_field($array)
{
	foreach ($array as $key => &$value) {
		if (is_array($value)) {
			...
			$value = sanitize_text_field(str_replace(array('{{', '}}'), '', wp_strip_all_tags($value)));
		}
	}
...
function b2_str_ends_with($haystack, $needle)
{
...
function b2_redirect_404($post_type)
{

	$allows = ['document', 'circle', 'infomation', 'shop', 'links', 'newsflashes', 'circle', 'ask'];

	if (in_array($post_type, $allows)) {
		if ($post_type == 'links') {
			$open = b2_get_option('links_main', 'link_open');
		} else {
			$open = b2_get_option($post_type . '_main', $post_type . '_open');
		}
...
function b2_get_decimal_length($number)
{
    $number = (string) $number;
    $decimalPosition = strpos($number, '.');
    return $decimalPosition === false ? 0 : strlen(substr($number, $decimalPosition + 1));
}


function b2_add($a, $b)
{
    $multiplier = pow(10, max(b2_get_decimal_length($a), b2_get_decimal_length($b)));
    return round(($a * $multiplier + $b * $multiplier) / $multiplier, 2);
}

function b2_sub($num1, $num2)
{
    $multiplier = pow(10, max(b2_get_decimal_length($num1), b2_get_decimal_length($num2)));
    return round(($num1 * $multiplier - $num2 * $multiplier) / $multiplier, 2);
}

function b2_mul($num1, $num2)
{
    $multiplier = pow(10, b2_get_decimal_length($num1) + b2_get_decimal_length($num2));
    return round(($num1 * pow(10, b2_get_decimal_length($num1)) * ($num2 * pow(10, b2_get_decimal_length($num2)))) / $multiplier, 2);
}

function b2_div($num1, $num2)
{
    $multiplier = pow(10, b2_get_decimal_length($num1) - b2_get_decimal_length($num2));
    return round(($num1 * pow(10, b2_get_decimal_length($num1))) / ($num2 * pow(10, b2_get_decimal_length($num2))) / $multiplier, 2);
}
