...
class Normal
{
    ...
    public static $default_settings = array(
        ...
        'custom_activity_name' => '活动',
        'custom_activity_link' => 'activity',
    ...
    public function normal_options_page()
    {
        ...
        // $custom->add_field(array(
        //     'before_row'=>'<h2>自定义活动</h2>',
        //     'name'=>__('【活动】名称','b2'),
        //     'id'=>'custom_activity_name',
        //     'type'=>'text',
        //     'default'=>self::$default_settings['custom_activity_name']
        // ));

        // $custom->add_field(array(
        //     'name'=>__('【活动】连接别名','b2'),
        //     'id'=>'custom_activity_link',
        //     'type'=>'text',
        //     'default'=>self::$default_settings['custom_activity_link']
        // ));
        ...
        $safe->add_field(array(
            ...
            'desc' => sprintf(
                __('站点中所有用户需要输入的文本都将经过AI文本审查功能进行过滤，避免涉黄、涉敏感、涉广告等违规内容的发布。%s1、请前往%s百度AI开放平台%s开通账户。%s2、控制面板的【内容审核】中创建应用，只勾选内容审核相关的。%s3、将生成的appid，API Key，Secret Key填入下方。%s免费用户拥有5万次的免费API调用，每秒并发2QPS', 'b2')
                ,
                '<br>',
        ...
        $login->add_field(array(
            'name' => __('是否只开启社交登录', 'b2'),
            'desc' => __('开启后，用户只能通过社交登录，不能通过用户名密码登录。', 'b2'),
            'id' => 'allow_social_login',
            'type' => 'select',
            'default' => self::$default_settings['allow_social_login'],
            'options' => array(
                1 => __('开启', 'b2'),
                0 => __('关闭', 'b2'),
            ),
        ...
        //腾讯云人机验证设置
        $login->add_field(array(
            'before_row' => '<div id="tencent" class="cmb-row"><h5>' . __('腾讯云人机验证设置', 'b2') . '</h5>',
            'name' => __('腾讯云SecretId', 'b2'),
            'desc' => __('腾讯云右上角用户名下拉菜单->访问管理->访问密钥->API密钥管理->SecretId', 'b2'),
            'id' => 'tencent_secret_id',
            'type' => 'text',
            'default' => isset(self::$default_settings['tencent_secret_id']) ? self::$default_settings['tencent_secret_id'] : '',
        ));

        $login->add_field(array(
            'name' => __('腾讯云SecretKey', 'b2'),
            'desc' => __('腾讯云右上角用户名下拉菜单->访问管理->访问密钥->API密钥管理->SecretKey', 'b2'),
            'id' => 'tencent_secret_key',
            'type' => 'text',
            'default' => isset(self::$default_settings['tencent_secret_key']) ? self::$default_settings['tencent_secret_key'] : '',
        ));

        $login->add_field(array(
            'name' => __('CaptchaAppId', 'b2'),
            'desc' => __('腾讯云控制台->人机验证->图形验证->验证管理。', 'b2'),
            'id' => 'CaptchaAppId',
            'type' => 'text',
            'default' => isset(self::$default_settings['CaptchaAppId']) ? self::$default_settings['CaptchaAppId'] : '',
        ));

        $login->add_field(array(
            'name' => __('AppSecretKey', 'b2'),
            'desc' => __('腾讯云控制台->人机验证->图形验证->验证管理。', 'b2'),
            'id' => 'AppSecretKey',
            'type' => 'text',
            'default' => isset(self::$default_settings['AppSecretKey']) ? self::$default_settings['AppSecretKey'] : '',
            'after_row' => '<p class="red">申请地址：<a href="https://console.cloud.tencent.com/captcha/graphical" target="__blank">腾讯云人机验证</a></p></div>',
        ));
        ...
        $login->add_field(array(
            ...
            'before_row' => '<h2>' . __('微信登录设置', 'b2') . '</h2><div class="cmb-row"><p><b>' . __('优先微信登录', 'b2') . '</b></p>',
            'id' => 'wx_show_qrcode',
            'type' => 'select',
            'options' => array(
                1 => __('开启', 'b2'),
            ...
            'desc' => __('开启后，登录窗口优先显示微信登陆二维码（只对微信官方有效，彩虹聚合登录不支持直接显示二维码）', 'b2'),
            'after_row' => '</div>',
        ...
        $login->add_field(array(
            ...
            'before_row' => '<div class="cmb-row"><p><b>' . __('微信pc端登录设置', 'b2') . '</b></p>',
            'id' => 'wx_pc_open',
            'type' => 'select',
            'options' => array(
                1 => __('开启', 'b2'),
        ...
        $login->add_field(array(
            ...
            'id' => 'wx_pc_secret',
            'desc' => __('微信开放平台网站应用的AppSecret', 'b2'),
            str_replace(array('http', 'https'), '', B2_HOME_URI),
            'type' => 'text',
            'attributes' => array(
                'type' => 'password',
            ),
            'default' => self::$default_settings['wx_pc_secret'],
            'after_row' => '<p class="red">' . sprintf(
                __('微信扫码登录申请地址：%s；回调地址请填写：%s', 'b2'),
                '<a target="_blank" href="https://open.weixin.qq.com/">https://open.weixin.qq.com/</a>',
                str_replace(array('http://', 'https://'), '', B2_HOME_URI)
            ) . '</p></div>'
        ));
        ...
        $user->add_field(array(
            'name' => __('是否显示用户描述', 'b2'),
            'desc' => __('经常有人利用用户描述来填写广告和垃圾信息，如果您用不上用户描述，建议关闭', 'b2'),
            'id' => 'show_desc',
            'type' => 'select',
            'options' => array(
                1 => __('显示', 'b2'),
            ...
            'default' => self::$default_settings['show_desc'],
        ));
    ...
    public static function gold_settings()
    {
        ...
        $gold->add_field(array(
            ...
            'default' => 0,
            'before_row' => '<h2>' . __('余额设置', 'b2') . '</h2><p class="red">' . __('请务必要做一定的限制，否则接口可能会被坏人利用。') . '</p>'
        ));

        $gold->add_field(array(
            'name' => __('每天最大的充值次数', 'b2'),
            'id' => 'gold_cz_count',
            'type' => 'text',
            'default' => 2,
            'description' => __('填写数字，比如填2，则限制每天只能充值两次', 'b2')
        ));

        $gold->add_field(array(
            'name' => __('每次充值的最大金额', 'b2'),
            'id' => 'gold_cz_max',
            'type' => 'text',
            'default' => 100,
            'description' => __('每次充值的金额不能大于此数，请直接填数字，比如100，则每次充值不允许超过100', 'b2')
        ));
        ...
        $gold->add_field(array(
            ...
            'default' => 0,
            'before_row' => '<h2>' . __('积分兑换', 'b2') . '</h2><p class="red">' . __('请务必要做一定的限制，否则接口可能会被坏人利用。') . '</p>'
        ));

        $gold->add_field(array(
            'name' => __('每天最大的购买次数', 'b2'),
            'id' => 'gold_cg_count',
            'type' => 'text',
            'default' => 2,
            'description' => __('填写数字，比如填2，则限制每天只能购买两次', 'b2')
        ));

        $gold->add_field(array(
            'name' => __('每次购买的最大金额', 'b2'),
            'id' => 'gold_cg_max',
            'type' => 'text',
            'default' => 100,
            'description' => __('每次购买的金额不能大于此数，请直接填数字，比如100，则每次购买不允许超过100', 'b2')
        ));
        ...
        $gold->add_field(array(
            ...
            'before_row' => '<p>为了避免用户故意刷积分的问题存在，暂时只限制允许购买积分，不允许积分兑换' . B2_MONEY_NAME . '</p>'
        ));
    ...
    public static function weixin_message()
    {
        ...
        $weixin->add_field(array(
            ...
            <p>3、请确保已经开通了公众号<模板消息>功能。如果未开通，请前往微信公众号后台<a href="https://mp.weixin.qq.com/" target="_blank">公众号后台</a>，左侧菜单最下面，点击 新功能，找到模板消息，申请开通。申请的时候请确保选择 商业服务  >  软件/建站/技术开发 行业</p>
